...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V10.5",
    page_icon="🔧",
    layout="wide",
...
.block-container{padding-top:1.35rem;padding-bottom:5rem;max-width:1220px}
...
.hero-title{font-size:1.34rem;font-weight:820;letter-spacing:-.03em;color:#f8fafc!important}
.hero-sub{color:#d5dde8!important;font-size:.9rem;margin-top:.18rem;font-weight:600}
.chips{display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.65rem}
.chip{
  color:#eef3f8!important;font-size:.82rem;font-weight:700;
  padding:.32rem .52rem;border-radius:999px;
  border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.09)
}
.hero .chip,.hero .chip *{color:#eef3f8!important}
div[role="radiogroup"] label,div[role="radiogroup"] label p{color:#eaf0f6!important;font-weight:650}
div[data-testid="stMetricLabel"],div[data-testid="stMetricLabel"] p{color:#cbd5e1!important}
div[data-testid="stMetricValue"],div[data-testid="stMetricValue"] *{color:#f8fafc!important}
[data-testid="stCaptionContainer"],[data-testid="stCaptionContainer"] p{color:#c7d0dc!important}
...
@media(max-width:700px){
  .block-container{padding-left:.56rem;padding-right:.56rem;padding-top:1.15rem}
...
# ---------------- VIN / OPTION FITMENT ----------------
# NHTSA vPIC can decode VIN-coded trim/series/drivetrain/safety variables, but
# it does not prove every as-built option. V10.4 therefore uses conservative
# donor-level gates for premium/option-sensitive parts.
...
ENGINE_SENSITIVE_PARTS={"ECU / ECM / PCM","Turbocharger","Diesel High Pressure Fuel Pump","Diesel Injector Set"}
DIESEL_ENGINE_PARTS={"Diesel High Pressure Fuel Pump","Diesel Injector Set"}
TURBO_ENGINE_PARTS={"Turbocharger"}
ENGINE_VALUE_EXCEPTIONS={"Turbocharger","Diesel High Pressure Fuel Pump","Diesel Injector Set"}
...
def _contains_word(text,term):
    t=_upper_words(text); q=_upper_words(term)
    return bool(re.search(r"(?:^| )"+re.escape(q)+r"(?: |$)",t))

def _vin_trim_text(profile):
    if not profile: return ""
    return " ".join(str(profile.get(k) or "") for k in ("trim","trim2","series","series2")).strip()

def _vin_base_trim(profile):
    t=_vin_trim_text(profile)
    return any(_contains_word(t,x) for x in BASE_TRIM_TERMS)

def _vin_premium_trim(profile):
    t=_vin_trim_text(profile)
    return any(_contains_word(t,x) for x in PREMIUM_TRIM_TERMS)

def _vin_feature(profile,*keys):
...
def _feature_standard(v):
    t=_upper_words(v)
    return t in {"STANDARD","YES","STD"} or t.startswith("STANDARD ")

def _feature_unavailable(v):
    return _upper_words(v) in {"NOT AVAILABLE","NO","N A","NA"}

def refresh_cloud_vin_cache(force=False):
    cache=st.session_state.setdefault("_vin_decode_cache",{})
    now_ts=time.time()
    last=float(st.session_state.get("_vin_decode_cache_ts",0) or 0)
    if cache and not force and (now_ts-last)<900:
        return cache
    rows,status=background_rest_rows("scanner_vin_decode",{
    ...
    if rows:
        cache={str(r.get("vin") or "").upper():r for r in rows if r.get("vin")}
        st.session_state["_vin_decode_cache"]=cache
        st.session_state["_vin_decode_cache_ts"]=now_ts
        st.session_state["_vin_decode_status"]="OK"
    else:
        st.session_state["_vin_decode_status"]=status
    return st.session_state.get("_vin_decode_cache",{})

def vin_profile(vehicle):
    vin=str(vehicle.get("vin") or vehicle.get("VIN") or "").strip().upper()
    if len(vin)!=17: return None
    return refresh_cloud_vin_cache().get(vin)


def engine_profile(vehicle):
    prof=vin_profile(vehicle) or {}
    decoded=prof.get("decoded") or {}
    if not isinstance(decoded,dict):
        decoded={}
    return {
        "profile":prof,
        "model":str(prof.get("engine_model") or decoded.get("EngineModel") or "").strip(),
        "fuel":str(prof.get("fuel_type") or decoded.get("FuelTypePrimary") or "").strip(),
        "cylinders":str(prof.get("engine_cylinders") or decoded.get("EngineCylinders") or "").strip(),
        "displacement_l":str(prof.get("displacement_l") or decoded.get("DisplacementL") or "").strip(),
        "turbo":str(prof.get("turbo") or decoded.get("Turbo") or "").strip(),
    }
...
def _engine_no(v):
    return _upper_words(v) in {"NO","N","FALSE","0","NOT AVAILABLE","N A","NA"}

def engine_is_diesel(vehicle):
    e=engine_profile(vehicle)
    return "DIESEL" in _upper_words(e["fuel"]) or "DIESEL" in _upper_words(e["model"])

def engine_is_turbo(vehicle):
    e=engine_profile(vehicle)
    if _engine_yes(e["turbo"]):
        return True
    if _engine_no(e["turbo"]):
        return False
    prof=e["profile"]
    txt=" ".join([e["model"],str(prof.get("series") or ""),str(prof.get("trim") or "")])
    return any(t in _upper_words(txt) for t in (
        "TURBO","TURBOCHARGED","ECOBOOST","TDI","TFSI","TSI",
        "POWER STROKE","POWERSTROKE","DURAMAX","ECODIESEL"
    ))


def engine_common_rail_likely(vehicle):
    e=engine_profile(vehicle)
    if not engine_is_diesel(vehicle):
        return False
    y=int(vehicle.get("year") or 0)
    make=_upper_words(vehicle.get("make"))
    model=_upper_words(vehicle.get("model"))
    prof=e["profile"]
    txt=_upper_words(" ".join([e["model"],str(prof.get("series") or ""),model]))
    ...
    if "FORD" in make and any(t in model for t in ("F 250","F 350","F 450","F 550","SUPER DUTY")):
        return any(t in txt for t in ("6 4","6 7")) or y>=2008
...
def engine_display(vehicle):
    e=engine_profile(vehicle)
    bits=[]
    disp_term=f'{e["displacement_l"]}L' if e["displacement_l"] else ""
    if disp_term and disp_term.upper() not in str(e["model"]).upper():
    ...
    if e["fuel"]:
        bits.append(e["fuel"])
    if e["turbo"]:
        bits.append(f'Turbo {e["turbo"]}')
    return " · ".join(dict.fromkeys(bits))

def discovery_family_id_local(vehicle):
    profile=vin_profile(vehicle)
    trim=_upper_words((profile or {}).get("trim") or (profile or {}).get("series") or "")
    ep=engine_profile(vehicle)
    raw_engine="|".join(_upper_words(ep.get(k,"")) for k in ("fuel","model","displacement_l","cylinders","turbo")) or "ENGINE UNKNOWN"
    raw="|".join([str(int(_safe_float(vehicle.get("year"),0))),_upper_words(vehicle.get("make")),_upper_words(vehicle.get("model")),raw_engine,trim])
    return hashlib.sha1(raw.lower().encode()).hexdigest()

def engine_market_suffix(vehicle):
    e=engine_profile(vehicle)
    vals=[e["fuel"],e["model"],e["displacement_l"],e["cylinders"],e["turbo"]]
    return "|".join(_upper_words(x) for x in vals)

def engine_part_fitment(vehicle,part):
    """Hard engine gate: unknown/nonmatching engines do not get engine-specific recommendations."""
    if part not in ENGINE_SENSITIVE_PARTS:
        return {"allowed":True,"status":"Engine not required","reason":"Standard/non-engine-sensitive","confidence":1.0}
    e=engine_profile(vehicle)
    if not e["profile"]:
        return {"allowed":False,"status":"Engine unknown","reason":"No cloud VIN engine decode yet","confidence":0.0}
    label=engine_display(vehicle) or "decoded engine"
    if part=="Diesel High Pressure Fuel Pump":
        ok=engine_common_rail_likely(vehicle)
        return {
            "allowed":ok,
            "status":"Engine-confirmed" if ok else "Fuel-system mismatch",
            "reason":f"{'VIN/year/model support a common-rail diesel HPFP' if ok else 'Diesel is not confirmed as a compatible common-rail HPFP application'} ({label})",
            "confidence":.96,
        }
    if part=="Diesel Injector Set":
        ok=engine_is_diesel(vehicle)
        return {
            "allowed":ok,
            "status":"Engine-confirmed" if ok else "Fuel mismatch",
            "reason":f"VIN {'confirms diesel powertrain' if ok else 'does not confirm diesel'} ({label})",
            "confidence":.98,
        }
    if part in TURBO_ENGINE_PARTS:
        ok=engine_is_turbo(vehicle)
        return {
            "allowed":ok,
            "status":"Engine-confirmed" if ok else "Turbo mismatch",
            "reason":f"VIN {'confirms/strongly identifies turbocharging' if ok else 'does not confirm turbocharging'} ({label})",
            "confidence":.95,
        }
    return {"allowed":False,"status":"Engine unproven","reason":"Engine-specific part not confirmed","confidence":0.0}

def vin_part_fitment(vehicle,part):
    """Conservative donor-level fitment for option-sensitive parts."""
    if part not in OPTION_SENSITIVE_PARTS:
        return {"allowed":True,"status":"VIN not required","reason":"Standard/non-option-sensitive","confidence":1.0,"trim":"","series":""}

    profile=vin_profile(vehicle)
    trim=_vin_trim_text(profile)
    series=str((profile or {}).get("series") or "")
    strict=bool(st.session_state.get("strict_vin_fitment",True))

    if not profile:
        allowed=not (strict and part in STRICT_DECODE_PARTS)
        return {"allowed":allowed,"status":"VIN unknown","reason":"No cloud VIN decode yet; physically verify option","confidence":0.2,"trim":"","series":""}

    base=_vin_base_trim(profile); premium=_vin_premium_trim(profile)
    ent=_vin_feature(profile,"EntertainmentSystem")

    if part=="OEM Navigation Screen":
        if "NAV" in _upper_words(ent):
            out=(True,"VIN-supported","NHTSA entertainment data references navigation",.95)
        else:
            out=(False,"Package unproven",f"VIN decoded {trim or 'trim unknown'} but does not prove factory navigation; verify the donor physically",.25)
    elif part=="Amplifier":
        ent_u=_upper_words(ent)
        if any(_upper_words(x) in ent_u for x in PREMIUM_AUDIO_TERMS):
            out=(True,"VIN-supported","Decoded entertainment data indicates premium audio",.95)
        else:
            out=(False,"Audio package unproven","VIN does not prove a separate premium-audio amplifier; verify the donor physically",.25)
    elif part=="Power Folding Mirror":
        out=(False,"Mirror option unproven","VIN does not reliably prove power-fold mirrors; verify the donor physically",.25)
    elif part=="Radar Sensor":
        vals=[_vin_feature(profile,"AdaptiveCruiseControl"),_vin_feature(profile,"ForwardCollisionWarning"),
              _vin_feature(profile,"DynamicBrakeSupport"),_vin_feature(profile,"CIB")]
        if any(_feature_standard(v) for v in vals if v):
            out=(True,"VIN-supported","Decoded active-safety equipment supports a forward radar sensor",.9)
        elif vals and all(_feature_unavailable(v) for v in vals if v):
            out=(False,"Safety mismatch","Decoded radar-related safety systems are unavailable",.9)
        else:
            out=(False if strict else True,"Radar option unproven","VIN data does not confirm a radar-equipped package",.3)
    elif part=="Camera / ADAS Module":
        vals=[_vin_feature(profile,"LaneDepartureWarning"),_vin_feature(profile,"LaneKeepSystem"),
              _vin_feature(profile,"BackupCamera"),_vin_feature(profile,"ParkingAssist"),
              _vin_feature(profile,"ForwardCollisionWarning")]
        if any(_feature_standard(v) for v in vals if v):
            out=(True,"VIN-supported","Decoded camera/safety equipment supports an ADAS/camera module",.9)
        elif vals and all(_feature_unavailable(v) for v in vals if v):
            out=(False,"Safety mismatch","Decoded camera/ADAS systems are unavailable",.9)
        else:
            out=(False if strict else True,"ADAS option unproven","VIN data does not confirm the relevant camera/ADAS package",.3)
    elif part in {"LED Headlight Assembly","LED Tail Light Assembly"}:
        out=(False,"Lamp technology unproven",f"VIN trim {trim or 'decoded'} does not prove LED vs halogen/incandescent; verify the donor physically",.3)
    else:
        out=(True,"Verify option","Option-sensitive part; physical verification required",.4)

    return {"allowed":bool(out[0]),"status":out[1],"reason":out[2],"confidence":float(out[3]),
            "trim":str((profile or {}).get("trim") or ""),"series":series}
...
HARD_NO_PULL_TERMS=[
    "engine assembly","complete engine","long block","short block",
    "transmission","transaxle",
]

DEFAULT_BULKY_NO_PULL_TERMS=[
    "transfer case","differential","axle assembly","rear axle","front axle",
    "tailgate","liftgate","door assembly","hood","bumper","bed assembly",
]

def quick_pull_check(part_name,weight=None,length=None,width=None,height=None,pull_minutes=None,ship_est=None):
    """
    Decide whether a part is suitable for the default small/quick-pull strategy.
    Engines and complete transmissions are always blocked from recommendations.
    Other bulky/time-consuming items are blocked while Quick-Pull Only is enabled.
    """
    n=norm(part_name)

    if any(t in n for t in HARD_NO_PULL_TERMS):
        return False,"Hard excluded: complete engine/transmission"

    if not bool(st.session_state.get("quick_pull_only",True)):
        return True,"Quick-Pull filter off"

    if any(t in n for t in DEFAULT_BULKY_NO_PULL_TERMS):
        return False,"Bulky drivetrain/body part"

    try:
        limit=float(st.session_state.get("max_pull_minutes",60))
        # Confirmed engine-value parts get a modest time exception. They still
        # have to earn their place through profit/hour and DWMT scoring.
        if part_name in ENGINE_VALUE_EXCEPTIONS:
            limit=max(limit,110.0)
        if pull_minutes is not None and float(pull_minutes)>limit:
            return False,f"Pull time > {int(limit)} min"
    except Exception:
        pass

    try:
        if weight is not None and float(weight)>float(st.session_state.get("max_part_weight",80)):
            return False,f"Weight > {int(st.session_state.get('max_part_weight',80))} lb"
    except Exception:
        pass

    try:
        longest=max(float(length or 0),float(width or 0),float(height or 0))
        if longest>float(st.session_state.get("max_part_dimension",48)):
            return False,f"Dimension > {int(st.session_state.get('max_part_dimension',48))} in"
    except Exception:
        pass

    try:
        if ship_est is not None and float(ship_est)>float(st.session_state.get("max_ship_estimate",75)):
            return False,f"Shipping estimate > ${int(st.session_state.get('max_ship_estimate',75))}"
    except Exception:
        pass

    return True,"Quick-pull approved"

def quick_pull_from_part_row(p):
    ship,_=shipping_estimate(float(p.weight),float(p.length),float(p.width),float(p.height))
    return quick_pull_check(
        p.part,p.weight,p.length,p.width,p.height,p.pull_minutes,ship
    )

def filter_recommendable(df):
    if df is None or len(df)==0:
        return df
    if "RECOMMENDABLE" in df.columns:
        return df[df["RECOMMENDABLE"].astype(bool)].copy()
    return df.copy()
...
def applicability(vehicle,p):
    ...
    if rule=="diesel_likely":
        prof=vin_profile(vehicle)
        if prof: return 1.0 if engine_is_diesel(vehicle) else 0.0
        return .12 if any(k in text for k in DIESEL_HINTS) else .0
    if rule=="turbo_likely":
        prof=vin_profile(vehicle)
        if prof: return 1.0 if engine_is_turbo(vehicle) else 0.0
        return .12 if any(k in text for k in TURBO_HINTS) else .0
    return .5
...
def comp_query(vehicle,part,pn=""):
    term={
        ...
        "Turbocharger":"OEM turbocharger",
        "Diesel High Pressure Fuel Pump":"OEM diesel high pressure fuel injection pump",
        "Diesel Injector Set":"OEM diesel fuel injector set",
    ...
    if part in ENGINE_SENSITIVE_PARTS:
        ...
        ebits=[]
        disp_term=f'{e["displacement_l"]}L' if e["displacement_l"] else ""
        if disp_term and disp_term.upper() not in str(e["model"]).upper(): ebits.append(disp_term)
        if e["model"]: ebits.append(e["model"])
        if e["fuel"]: ebits.append(e["fuel"])
        if ebits: engterm=" "+" ".join(dict.fromkeys(ebits))
    return f"{vehicle.get('year')} {vehicle.get('make')} {vehicle.get('model')}{engterm} {term}{pnterm} used"


def _local_disp_float(v):
    try:
        x=float(str(v).strip())
        return x if .5<=x<=10 else None
    except Exception:
        m=re.search(r"(\d(?:\.\d+)?)",str(v or ""))
        return float(m.group(1)) if m else None

def _local_title_disps(title):
    vals=[]
    for m in re.finditer(r"(?<!\d)([1-9](?:\.\d)?)\s*[lL]\b",str(title or "")):
        try: vals.append(float(m.group(1)))
        except Exception: pass
    return vals

def title_matches_vehicle_engine(title,vehicle,strict=False):
    """Reject local eBay/Shopping comps that explicitly belong to a different engine."""
    e=engine_profile(vehicle)
    if not e or not any(str(e.get(k) or "").strip() for k in ("displacement_l","model","fuel","cylinders")):
        return not strict
    t=norm(title)
    expected=_local_disp_float(e.get("displacement_l"))
    found=_local_title_disps(title)
    strong=False
    if expected and found:
        if not any(abs(x-expected)<=.16 for x in found):
            return False
        strong=True
    try: cyl=int(float(str(e.get("cylinders") or "0")))
    except Exception: cyl=0
    found_c=[int(x) for x in re.findall(r"\b([3-8])\s*(?:cyl|cylinder|cylinders)\b",str(title or ""),re.I)]
    if cyl and found_c:
        if cyl not in found_c: return False
        strong=True
    fuel=norm(e.get("fuel",""))
    diesel="diesel" in fuel
    if diesel and any(x in f" {t} " for x in [" gasoline "," gas engine "," petrol "]): return False
    if (not diesel) and any(x in f" {t} " for x in [" diesel "," tdi "," cdi "," duramax "," power stroke "," ecodiesel "]): return False
    model=norm(e.get("model",""))
    model_tokens=[
        x for x in re.findall(r"[a-z0-9][a-z0-9.-]{2,}",model)
        if any(c.isalpha() for c in x) and any(c.isdigit() for c in x)
    ]
    if model_tokens and any(tok in t for tok in model_tokens):
        strong=True
    return strong if strict else True

def filter_comps_for_engine(df,vehicle,part):
    if df is None or not len(df) or "title" not in df.columns:
        return df
    strict=part in ENGINE_SENSITIVE_PARTS
    keep=df["title"].astype(str).map(lambda x:title_matches_vehicle_engine(x,vehicle,strict))
    return df[keep].copy()
...
# ---------------- FULL LKQ CATALOG x EBAY SWEEP ----------------
CATALOG_EXCLUDED_RECOMMENDATION_TERMS=[
    "engine","long block","short block","transmission","transaxle",
    "transfer case","differential","axle assembly","rear axle","front axle",
    "tailgate","liftgate","door assembly","hood","bumper","bed assembly",
    "wheel","rim","tire",
]
CATALOG_NONPART_TERMS=[
    "admission","environmental fee","warranty","guarantee only","core only",
    "miscellaneous fee","battery charge","sales tax",
]

def catalog_price_rows():
    """
    Every usable row currently loaded from the Wichita LKQ/PYP price list.
    The sweep still searches excluded categories, but flags things like engines
    and wheels so they do not appear in recommended-pull rankings.
    """
    rows=[]
    for name,v in (st.session_state.get("live_prices") or {}).items():
        if not isinstance(v,dict):
            continue
        nm=re.sub(r"\s+"," ",str(name)).strip().upper()
        if not nm:
            continue
        if any(t in norm(nm) for t in CATALOG_NONPART_TERMS):
            continue
        price=safe_float(v.get("price"),np.nan)
        total=safe_float(v.get("total"),np.nan)
        core=safe_float(v.get("core"),0.0)
        guarantee=safe_float(v.get("guarantee"),0.0)
        if pd.isna(price) or price<=0:
            # If a page only exposes a total, keep it as the comparison cost.
        ...
        rows.append({
            "LKQ_PART":nm,
            "LKQ_PRICE":float(price),
            "LKQ_TOTAL":float(total) if pd.notna(total) else float(price),
            "CORE":float(core),
            "GUARANTEE":float(guarantee),
        })
    return pd.DataFrame(rows).sort_values("LKQ_PART").reset_index(drop=True) if rows else pd.DataFrame(
        columns=["LKQ_PART","LKQ_PRICE","LKQ_TOTAL","CORE","GUARANTEE"]
    )

def catalog_donor_families():
    """Unique year/make/model groups; one eBay search can serve every matching donor."""
    inv=st.session_state.get("inventory")
    if inv is None or not isinstance(inv,pd.DataFrame) or len(inv)==0:
        return pd.DataFrame(columns=["year","make","model","DONORS"])
    x=inv.copy()
    for c in ["year","make","model"]:
        if c not in x.columns:
            x[c]=""
    x["year"]=x["year"].astype(str).str.replace(r"\.0$","",regex=True)
    x["make"]=x["make"].astype(str).str.upper().str.strip()
    x["model"]=x["model"].astype(str).str.upper().str.strip()
    x=x[(x["year"]!="")&(x["make"]!="")&(x["model"]!="")]
    return (x.groupby(["year","make","model"],dropna=False)
             .size().reset_index(name="DONORS")
             .sort_values(["year","make","model"])
             .reset_index(drop=True))

def catalog_sweep_signature():
    fam=catalog_donor_families()
    parts=catalog_price_rows()
    raw="||".join(
        ["|".join(fam[["year","make","model"]].astype(str).agg("|".join,axis=1).tolist()),
         "|".join(parts["LKQ_PART"].astype(str).tolist())]
    )
    return hashlib.sha256(raw.encode()).hexdigest()[:20]

def catalog_total_queries():
    return len(catalog_donor_families())*len(catalog_price_rows())

def catalog_query_from_index(flat_index):
    fam=catalog_donor_families()
    parts=catalog_price_rows()
    pcount=len(parts)
    if pcount==0 or len(fam)==0:
        return None
    total=len(fam)*pcount
    if flat_index<0 or flat_index>=total:
        return None
    fi=int(flat_index)//pcount
    pi=int(flat_index)%pcount
    f=fam.iloc[fi]
    p=parts.iloc[pi]
    query=f"{f.year} {f.make} {f.model} OEM {p.LKQ_PART} used"
    return {
        "INDEX":int(flat_index),
        "YEAR":str(f.year),"MAKE":str(f.make),"MODEL":str(f.model),
        "DONORS":int(f.DONORS),
        "LKQ_PART":str(p.LKQ_PART),
        "LKQ_PRICE":float(p.LKQ_PRICE),
        "LKQ_TOTAL":float(p.LKQ_TOTAL),
        "CORE":float(p.CORE),
        "GUARANTEE":float(p.GUARANTEE),
        "QUERY":query,
    }

def catalog_shipping_guess(part_name):
    """Coarse shipping allowance used only for ranking; user should verify before buying."""
    n=norm(part_name)
    local_terms=["door","hood","bumper","fender","decklid","tailgate","liftgate",
                 "transmission","transfer case","axle assembly","rear axle","differential"]
    if any(t in n for t in local_terms):
        return 175.0,"LOCAL/FREIGHT"
    if any(t in n for t in ["module","computer","ecm","pcm","bcm","switch","sensor",
                             "radio","display","cluster","amplifier","control","camera"]):
        return 13.0,"SMALL"
    if any(t in n for t in ["mirror","headlight","tail light","taillight","lamp"]):
        return 24.0,"MEDIUM"
    if any(t in n for t in ["starter","alternator","compressor","turbo","pump","caliper"]):
        return 29.0,"HEAVY"
    return 35.0,"ESTIMATE"

def catalog_recommendation_excluded(part_name,ship_est=None):
    n=norm(part_name)
    if any(t in n for t in CATALOG_EXCLUDED_RECOMMENDATION_TERMS):
        return True
    if bool(st.session_state.get("quick_pull_only",True)) and ship_est is not None:
        try:
            if float(ship_est)>float(st.session_state.get("max_ship_estimate",75)):
                return True
        except Exception:
            pass
    return False

def _catalog_ebay_worker(item,token,ebay_fee,quick_pull_only,max_ship_estimate):
    q=item["QUERY"]
    try:
        df=ebay_used_comps_with_token(q,token,30)
        summ=summarize_comps(df)
        median=summ.get("median",np.nan)
        n=int(summ.get("n",0) or 0)
        p25=summ.get("p25",np.nan)
        p75=summ.get("p75",np.nan)
        if pd.notna(median):
            ship,ship_class=catalog_shipping_guess(item["LKQ_PART"])
            fee=float(median)*float(ebay_fee)
            net=float(median)-fee-ship-float(item["LKQ_PRICE"])
            gross=float(median)-float(item["LKQ_PRICE"])
            roi=net/max(float(item["LKQ_PRICE"]),1)
        else:
            ship,ship_class=catalog_shipping_guess(item["LKQ_PART"])
            net=gross=roi=np.nan
        out={**item,
             "EBAY_MEDIAN":float(median) if pd.notna(median) else None,
             "EBAY_P25":float(p25) if pd.notna(p25) else None,
             "EBAY_P75":float(p75) if pd.notna(p75) else None,
             "EBAY_N":n,
             "SHIP_EST":float(ship),
             "SHIP_CLASS":ship_class,
             "EST_NET":float(net) if pd.notna(net) else None,
             "GROSS_SPREAD":float(gross) if pd.notna(gross) else None,
             "ROI":float(roi) if pd.notna(roi) else None,
             "EXCLUDED_REC":bool(any(t in norm(item["LKQ_PART"]) for t in CATALOG_EXCLUDED_RECOMMENDATION_TERMS) or (quick_pull_only and float(ship)>float(max_ship_estimate))),
        ...
        ship,ship_class=catalog_shipping_guess(item["LKQ_PART"])
        return {**item,
                "EBAY_MEDIAN":None,"EBAY_P25":None,"EBAY_P75":None,"EBAY_N":0,
                "SHIP_EST":ship,"SHIP_CLASS":ship_class,
                "EST_NET":None,"GROSS_SPREAD":None,"ROI":None,
                "EXCLUDED_REC":bool(any(t in norm(item["LKQ_PART"]) for t in CATALOG_EXCLUDED_RECOMMENDATION_TERMS) or (quick_pull_only and float(ship)>float(max_ship_estimate))),
                "SEARCHED_AT":_utc_now_iso(),"ERROR":str(e)}

def _compact_catalog_results(results,max_keep=3000):
    """
    Keep the most useful compact results in persistent app memory.
    Cursor progress still advances through every combination, so weak/no-result
    searches do not need to bloat Supabase JSON forever.
    """
    if not results:
        return []
    ...
    if not len(df):
        return []
    # Always keep profitable/valid rows first, then recent rows for diagnostics.
    valid=df[df["EBAY_MEDIAN"].notna()].copy() if "EBAY_MEDIAN" in df else pd.DataFrame()
    if len(valid):
        valid["_rank"]=pd.to_numeric(valid["EST_NET"],errors="coerce").fillna(-1e9)
        valid=valid.sort_values(["_rank","EBAY_N"],ascending=False).drop(columns=["_rank"])
    recent=df.tail(300)
    combined=pd.concat([valid.head(max_keep),recent],ignore_index=True)
    combined=combined.drop_duplicates(["YEAR","MAKE","MODEL","LKQ_PART"],keep="first").head(max_keep)
    return json_safe(combined.to_dict("records"))

def reset_catalog_sweep_if_needed(force=False):
    sig=catalog_sweep_signature()
    if force or st.session_state.get("catalog_sweep_signature","")!=sig:
        st.session_state.catalog_sweep_signature=sig
        st.session_state.catalog_sweep_cursor=0
        st.session_state.catalog_sweep_results=[]
        st.session_state.catalog_sweep_errors=0
        st.session_state.catalog_sweep_finished=False
        mark_memory_dirty("catalog sweep reset")
    return sig

def run_catalog_sweep_batch(batch_size=50,progress=None,status=None):
    cid,secret=ebay_credentials()
    if not (cid and secret):
        raise RuntimeError("Connect eBay in Streamlit Secrets first.")

    parts=catalog_price_rows()
    fam=catalog_donor_families()
    if len(parts)==0:
        raise RuntimeError("Sync Wichita LKQ prices first.")
    if len(fam)==0:
        raise RuntimeError("Sync Wichita inventory first.")

    reset_catalog_sweep_if_needed()
    pcount=len(parts)
    total=len(parts)*len(fam)
    cursor=int(st.session_state.get("catalog_sweep_cursor",0))
    if cursor>=total:
        st.session_state.catalog_sweep_finished=True
        return 0,total,total,[]

    stop=min(total,cursor+int(batch_size))
    tasks=[]
    for flat_index in range(cursor,stop):
        fi=flat_index//pcount
        pi=flat_index%pcount
        f=fam.iloc[fi]
        p=parts.iloc[pi]
        tasks.append({
            "INDEX":flat_index,
    ...
    token=ebay_access_token(cid,secret)
    workers=max(1,min(int(st.session_state.get("market_workers",8)),12,len(tasks)))
    ebay_fee=float(st.session_state.get("ebay_fee",.13))
    completed=0
    batch_results=[]
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        future_map={
            pool.submit(
                _catalog_ebay_worker,item,token,ebay_fee,
                bool(st.session_state.get("quick_pull_only",True)),
                float(st.session_state.get("max_ship_estimate",75))
            ):item
            for item in tasks
        }
        for fut in as_completed(future_map):
            ...
            if res.get("ERROR"):
                st.session_state.catalog_sweep_errors=int(st.session_state.get("catalog_sweep_errors",0))+1
            completed+=1
            if progress:
                progress.progress(completed/max(len(tasks),1))
            if status:
                status.info(
                    f"eBay catalog sweep: {cursor+completed:,} / {total:,} combinations · "
                    f"{workers} workers"
                )

    existing=list(st.session_state.get("catalog_sweep_results",[]))
    existing.extend(batch_results)
    st.session_state.catalog_sweep_results=_compact_catalog_results(existing,3000)
    st.session_state.catalog_sweep_cursor=stop
    st.session_state.catalog_sweep_finished=stop>=total
    mark_memory_dirty("eBay catalog sweep")
    return len(batch_results),stop,total,batch_results

def catalog_best_results(include_excluded=False,min_results=2):
    rows=st.session_state.get("catalog_sweep_results",[])
    if not rows:
        return pd.DataFrame()
    df=pd.DataFrame(rows)
    if not len(df):
        return df
    df["EBAY_N"]=pd.to_numeric(df.get("EBAY_N"),errors="coerce").fillna(0)
    df["EST_NET"]=pd.to_numeric(df.get("EST_NET"),errors="coerce")
    df["ROI"]=pd.to_numeric(df.get("ROI"),errors="coerce")
    df=df[(df["EBAY_N"]>=int(min_results))&df["EST_NET"].notna()]
    if not include_excluded and "EXCLUDED_REC" in df:
        df=df[~df["EXCLUDED_REC"].astype(bool)]
    return df.sort_values(["EST_NET","ROI","EBAY_N"],ascending=False).reset_index(drop=True)

def map_catalog_result_to_donors(row):
    inv=st.session_state.inventory
    if inv is None or not len(inv):
        return pd.DataFrame()
    mask=(
        inv["year"].astype(str).str.replace(r"\.0$","",regex=True).eq(str(row["YEAR"])) &
        inv["make"].astype(str).str.upper().eq(str(row["MAKE"]).upper()) &
        inv["model"].astype(str).str.upper().eq(str(row["MODEL"]).upper())
    )
...
def summarize_comps(df):
    if df is None or len(df)<3:
        return {"p25":np.nan,"median":np.nan,"raw_median":np.nan,"p75":np.nan,"n":0,
                "source_url":"","source_title":"","recommended":np.nan}
    x=df.copy()
    x["landed"]=pd.to_numeric(x["landed"],errors="coerce")
    x=x[x["landed"].between(8,8000)].dropna(subset=["landed"])
    if len(x)>=5:
        q1,q3=x["landed"].quantile([.25,.75]); iqr=q3-q1
        if iqr>0:
            x=x[x["landed"].between(max(8,q1-1.5*iqr),q3+1.5*iqr)]
    if len(x)<3:
        return {"p25":np.nan,"median":np.nan,"raw_median":np.nan,"p75":np.nan,"n":len(x),
                "source_url":"","source_title":"","recommended":np.nan}
    p25=float(x["landed"].quantile(.25))
    p35=float(x["landed"].quantile(.35))
    raw=float(x["landed"].median())
    p75=float(x["landed"].quantile(.75))
    spread=(p75-p25)/max(raw,1)*100
    rec=p25 if spread>85 else min(raw,p35)
    rec=min(rec,raw*.92)
    near=x.assign(_dist=(x["landed"]-rec).abs()).sort_values("_dist").iloc[0]
    return {
        "p25":p25,"median":float(rec),"raw_median":raw,"p75":p75,"n":len(x),
        "recommended":float(rec),
        "source_url":str(near.get("url","") or ""),
        "source_title":str(near.get("title","") or "")
    }
...
def market_key(vehicle,part,pn=""):
    eng=f"|{engine_market_suffix(vehicle)}" if part in ENGINE_SENSITIVE_PARTS else ""
    return f"{vehicle.get('year')}|{vehicle.get('make')}|{vehicle.get('model')}|{part}|{pn}{eng}"
...
def part_market(vehicle,p):
    ...
    if pd.notna(ebay.get("median",np.nan)):
        ...
        price_source_url=str(ebay.get("source_url") or "")
        price_source_title=str(ebay.get("source_title") or "")
        pricing_method=str(ebay.get("pricing_method") or "")
        price_confidence_grade=str(ebay.get("price_confidence_grade") or "")
        walkaway_reason=str(ebay.get("walkaway_reason") or "")
        if not price_source_url:
            ex=(ebay.get("examples") or [])
            if ex:
                price_source_url=str(ex[0].get("url") or "")
                price_source_title=str(ex[0].get("title") or "")
    ...
    comp_count,price_spread_pct,value_stability=market_quality_from_live(live)
    ...
    return {
        ...
        "market_sources":len(sources),"comp_count":comp_count,
        "price_spread_pct":price_spread_pct,"value_stability":value_stability,
        "price_source_url":price_source_url,"price_source_title":price_source_title,
        "pricing_method":pricing_method,"price_confidence_grade":price_confidence_grade,
...
def market_quality_from_live(live):
    """Compact quality metrics from the cached active-market observations."""
    live=live if isinstance(live,dict) else {}
    vals=[]; total_n=0; spreads=[]
    for provider in ("ebay","shopping"):
        p=live.get(provider,{}) if isinstance(live.get(provider,{}),dict) else {}
        med=_safe_float(p.get("median"),0)
        n=int(_safe_float(p.get("n"),0))
        if med>0:
            vals.append(med); total_n+=n
            p25=_safe_float(p.get("p25"),0); p75=_safe_float(p.get("p75"),0)
            if p25>0 and p75>=p25:
                spreads.append((p75-p25)/max(med,1))
    cross_spread=0.0
    if len(vals)>=2:
        cross_spread=(max(vals)-min(vals))/max(float(np.median(vals)),1)
    spread=max(spreads+[cross_spread,0.0])
    stability=max(0.0,100.0-min(spread,1.5)/1.5*100.0)
    return int(total_n),round(spread*100,1),round(stability,1)

def dwmt_score(p,expected_profit,pph,ship,confidence,present_pct,days_to_sell):
    """Don't Waste My Time score: high = profitable, quick, easy to ship, lower-risk."""
    exp=max(_safe_float(expected_profit),0)
    pph=max(_safe_float(pph),0)
    pull=max(_safe_float(getattr(p,"pull_minutes",0)),1)
    ship=max(_safe_float(ship),0)
    conf=np.clip(_safe_float(confidence),0,100)/100
    present=np.clip(_safe_float(present_pct),0,100)/100
    demand=np.clip(_safe_float(getattr(p,"demand_score",.5)),0,1)
    risk=np.clip(_safe_float(getattr(p,"return_risk",.2)),0,1)
    weight=max(_safe_float(getattr(p,"weight",0)),0)
    longest=max(_safe_float(getattr(p,"length",0)),_safe_float(getattr(p,"width",0)),_safe_float(getattr(p,"height",0)))
    sell_days=max(_safe_float(days_to_sell,45),1)
    warning=norm(getattr(p,"warnings",""))

    score=(
        np.clip(exp/250,0,1)*24 +
        np.clip(pph/300,0,1)*22 +
        conf*12 + present*10 + demand*8 +
        (1-np.clip(pull/90,0,1))*8 +
        (1-np.clip(ship/90,0,1))*5 +
        (1-np.clip(sell_days/120,0,1))*5 +
        (1-risk)*6
    )
    score-=np.clip(weight/120,0,1)*4
    score-=np.clip(longest/60,0,1)*3
    if any(t in warning for t in ["programming","calibration","immobilizer","mileage"]):
        score-=5
    return round(float(np.clip(score,0,100)),1)

def dwmt_label(score):
    s=_safe_float(score)
    if s>=80: return "🔥 GRAB IT"
    if s>=68: return "✅ EASY MONEY"
    if s>=55: return "👍 WORTH A LOOK"
    if s>=42: return "⚠️ ONLY IF CLEAN"
    return "⛔ SKIP / LOW PRIORITY"

def recommendation_snapshot(inv,limit=300):
    if inv is None or len(inv)==0:
        return pd.DataFrame()
    opp=filter_recommendable(get_opportunities_cached(inv))
    if opp is None or len(opp)==0:
        return pd.DataFrame()
    x=opp[(opp["MARKET_DATA"]==True)&(opp["PROFIT"]>0)].copy()
    if not len(x): return x
    x=x.sort_values(["DWMT_SCORE","EXPECTED_PROFIT","CONF"],ascending=False).head(int(limit))
    return x

def refresh_precomputed_recommendations():
    snap=recommendation_snapshot(st.session_state.get("inventory"),300)
    st.session_state.precomputed_recommendations=json_safe(snap.to_dict("records")) if len(snap) else []
    st.session_state.precomputed_at=_utc_now_iso()
    mark_memory_dirty("precomputed recommendations")
    return snap

def _append_price_history(key,entry):
    history=st.session_state.setdefault("price_history",[])
    ts=(entry or {}).get("updated_at") or _utc_now_iso()
    for provider in ("ebay","shopping"):
        p=(entry or {}).get(provider,{}) if isinstance((entry or {}).get(provider,{}),dict) else {}
        med=_safe_float(p.get("median"),0)
        if med<=0: continue
        history.append({
            "key":str(key),"provider":provider,"timestamp":ts,
            "median":med,"n":int(_safe_float(p.get("n"),0)),
            "p25":_safe_float(p.get("p25"),0),"p75":_safe_float(p.get("p75"),0),
        })
...
def background_rest_rows(table,params=None,limit=100):
    """Read V10 background tables with the existing Supabase publishable key."""
    url,key,_,_=supabase_config()
    if not (url and key): return [],"Supabase not configured"
    p=dict(params or {})
    p.setdefault("select","*")
    p.setdefault("limit",str(int(limit)))
    try:
        r=fast_http_session().get(
            f"{url}/rest/v1/{table}",headers={"apikey":key,"Accept":"application/json"},
            params=p,timeout=(4,8)
        )
        if r.status_code in (404,400):
            return [],"V10 background tables not installed yet"
        if not r.ok:
...
def background_rest_upsert(table,row,on_conflict=None):
    """Write a small V10.4 user verification row with the Supabase publishable key."""
    url,key,_,_=supabase_config()
    if not (url and key): return False,"Supabase not configured"
    ...
    try:
        r=fast_http_session().post(f"{url}/rest/v1/{table}",headers=headers,params=params,json=row,timeout=(4,10))
        if not r.ok: return False,f"HTTP {r.status_code}: {r.text[:220]}"
        return True,"Saved"
    except Exception as e:
        return False,str(e)


def background_rest_all_rows(table,params=None,page_size=1000,max_rows=20000):
    """Page through a Supabase REST table so cloud market backfill can feed the UI."""
    out=[]
    status="OK"
    base=dict(params or {})
    base.pop("limit",None); base.pop("offset",None)
    page_size=max(100,min(int(page_size),1000))
    for offset in range(0,int(max_rows),page_size):
        p=dict(base)
        p["limit"]=str(page_size)
        p["offset"]=str(offset)
        rows,status=background_rest_rows(table,p,page_size)
        if not rows:
            break
        out.extend(rows)
...
def refresh_cloud_run_summary(force=False):
    """Cheap latest cloud-worker status for the header and MARKET page."""
    now_ts=time.time()
    last=float(st.session_state.get("_cloud_run_status_ts",0) or 0)
    cached=st.session_state.get("_cloud_run_status",{})
    if cached and not force and (now_ts-last)<120:
        return cached
    rows,status=background_rest_rows("scanner_runs",{
        "select":"started_at,status,cars_found,new_cars,market_total,market_scanned,market_priced,market_coverage_pct,discovery_recommendations,error",
        "order":"started_at.desc","limit":"1"
    },1)
    if rows:
        st.session_state["_cloud_run_status"]=rows[0]
        st.session_state["_cloud_run_status_ts"]=now_ts
    else:
        st.session_state["_cloud_run_status_error"]=status
    return st.session_state.get("_cloud_run_status",{})

def refresh_cloud_market_cache(force=False):
    """Hydrate the Streamlit in-memory market cache from V10.4 Supabase rows.

    The background worker stores scanner_market_cache in a compact cloud schema,
    while the UI historically looked only at its local market_cache dictionary.
    This bridge makes BEST CARS, TODAY'S PLAN, header coverage and MARKET all use
    the background scanner's saved whole-yard work.
    """
    now_ts=time.time()
    last=float(st.session_state.get("_cloud_market_hydrated_at",0) or 0)
    if not force and last and (now_ts-last)<600 and st.session_state.get("_cloud_market_rows_loaded",0):
        return {
            "rows":int(st.session_state.get("_cloud_market_rows_loaded",0)),
            "priced":int(st.session_state.get("_cloud_market_priced_loaded",0)),
            "status":st.session_state.get("_cloud_market_hydrate_status","OK"),
        }

    url,key,_,_=supabase_config()
    if not (url and key):
        return {"rows":0,"priced":0,"status":"Supabase not configured"}

    rows,status=background_rest_all_rows("scanner_market_cache",{
    ...
    if not rows:
        st.session_state["_cloud_market_hydrate_status"]=status
        return {"rows":0,"priced":0,"status":status}

    inv=st.session_state.get("inventory")
    family_variants={}
    if isinstance(inv,pd.DataFrame) and len(inv):
        for _,v in inv.iterrows():
            y=int(_safe_float(v.get("year"),0))
            fam=(y,norm(v.get("make","")),norm(v.get("model","")))
            family_variants.setdefault(fam,set()).add(
                (v.get("year"),str(v.get("make","")),str(v.get("model","")))
            )

    local=st.session_state.setdefault("market_cache",{})
    priced=0
    touched=0
    for r in rows:
        year=int(_safe_float(r.get("year"),0))
        make=str(r.get("make") or "")
        model=str(r.get("model") or "")
        part=str(r.get("part") or "")
        if not year or not make or not model or not part:
            continue

        fam=(year,norm(make),norm(model))
        variants=family_variants.get(fam) or {(year,make,model)}
        if str(r.get("pricing_version") or "")!="10.5":
            continue
        med=r.get("recommended_sell_price")
        medf=_safe_float(med,0)
        if medf>0:
            priced+=1
        src_url=str(r.get("price_source_url") or "")
        src_title=str(r.get("price_source_title") or "")
        ebay={
            "median":medf if medf>0 else None,
            "raw_median":_safe_float(r.get("market_median"),0) or None,
            "n":int(_safe_float(r.get("market_n"),0)),
            "p25":_safe_float(r.get("market_p25"),0) or None,
            "p75":_safe_float(r.get("market_p75"),0) or None,
            "source_url":src_url,"source_title":src_title,
            "pricing_method":str(r.get("pricing_method") or ""),
            "price_confidence_grade":str(r.get("price_confidence_grade") or ""),
            "walkaway_reason":str(r.get("walkaway_reason") or ""),
            "evidence_json":r.get("evidence_json") or {},
            "examples":[{"title":src_title,"url":src_url,"landed":medf}] if src_url else [],
        }
        ts=str(r.get("updated_at") or _utc_now_iso())
        for yv,mv,modv in variants:
            if part in ENGINE_SENSITIVE_PARTS:
                eng=str(r.get("engine_key") or "").strip()
                key_local=f"{yv}|{mv}|{modv}|{part}||{eng}"
            else:
                key_local=f"{yv}|{mv}|{modv}|{part}|"

            entry=dict(local.get(key_local,{}) or {})
            entry["query"]=str(r.get("query") or entry.get("query") or "")
            entry["ebay"]=ebay
            timestamps=dict(entry.get("timestamps") or {})
            timestamps["ebay"]=ts
            entry["timestamps"]=timestamps
            entry["cloud_market_id"]=str(r.get("id") or "")
            entry["cloud_engine_label"]=str(r.get("engine_label") or "")
            entry["cloud_last_error"]=str(r.get("last_error") or "")
            local[key_local]=entry
            touched+=1

    st.session_state["_cloud_market_hydrated_at"]=now_ts
    st.session_state["_cloud_market_rows_loaded"]=len(rows)
    st.session_state["_cloud_market_priced_loaded"]=priced
    st.session_state["_cloud_market_hydrate_status"]=status
    recompute_market_covered_count_fast()
    invalidate_rankings()
    return {"rows":len(rows),"priced":priced,"status":status,"keys":touched}

def discovery_title_hard_blocked(title):
    """UI-side safety net for stale V10.4 discovery rows before cloud cleanup."""
    t=norm(title)
    hard=[
        "complete engine","engine assembly","engine motor","motor engine",
        "long block","short block","jdm engine","complete transmission",
        "transmission assembly","transmission transaxle",
        "wheel tire","wheel and tire","set of wheels","set of rims",
    ]
    if any(x in t for x in hard):
        return True
    if re.search(r"\b\d(?:\.\d)?l\b.{0,25}\bengine\b",t,re.I):
        return True
    if re.search(r"\bengine\b.{0,25}\b\d(?:\.\d)?l\b",t,re.I):
        return True
    return False
...
def opportunity(vehicle,p,allow_unpriced=False):
    ...
    efit=engine_part_fitment(vehicle,p.part)
    if p.part in ENGINE_SENSITIVE_PARTS and not efit["allowed"]:
        return None
    vfit=vin_part_fitment(vehicle,p.part)
    if bool(st.session_state.get("strict_vin_fitment",True)) and not vfit["allowed"]:
        return None
    m=part_market(vehicle,p)
    if st.session_state.market_only and not m["has_market_data"] and not allow_unpriced:
        return None
    if not allow_unpriced and str(m.get("walkaway_reason") or ""):
        return None
    ...
    score=(
        ...
        m["fit_bonus"]*20+
        present_prob*7+
        (float(vfit.get("confidence",1.0))*4 if p.part in OPTION_SENSITIVE_PARTS else 0) +
        (float(efit.get("confidence",1.0))*5 if p.part in ENGINE_SENSITIVE_PARTS else 0)
    )
    ...
    quick_ok,quick_reason=quick_pull_from_part_row(p)
    dwmt=dwmt_score(p,expected_profit,pph,ship,m["confidence"],present_pct,m["days_to_sell"])
    return {
        ...
        "DWMT_SCORE":dwmt,"DWMT":dwmt_label(dwmt),
        "VIN_FIT":vfit.get("status",""),"VIN_REASON":vfit.get("reason",""),
        "TRIM":vfit.get("trim","") or str((vin_profile(vehicle) or {}).get("trim") or ""),
        "SERIES":vfit.get("series","") or str((vin_profile(vehicle) or {}).get("series") or ""),
        "ENGINE":engine_display(vehicle),"FUEL":engine_profile(vehicle).get("fuel",""),
        "TURBO":engine_profile(vehicle).get("turbo",""),
        "ENGINE_FIT":efit.get("status",""),"ENGINE_REASON":efit.get("reason",""),
        "COMP_COUNT":int(m.get("comp_count",0)),"PRICE_SPREAD_PCT":m.get("price_spread_pct",0),
        "VALUE_STABILITY":m.get("value_stability",0),
        "RECOMMENDABLE":bool(quick_ok),"QUICK_PULL_REASON":quick_reason,
        "WEIGHT":float(p.weight),"LENGTH":float(p.length),"WIDTH":float(p.width),"HEIGHT":float(p.height),
        "WARNING":p.warnings,"LEARNED_WARNING":learned,
        "MARKET_DATA":m["has_market_data"],"MARKET_SOURCES":m["market_sources"],
        "PRICE_SOURCE_URL":m.get("price_source_url",""),
        "PRICE_SOURCE_TITLE":m.get("price_source_title",""),
        "PRICING_METHOD":m.get("pricing_method",""),
        "PRICE_CONFIDENCE":m.get("price_confidence_grade",""),
        "WALKAWAY_REASON":m.get("walkaway_reason",""),
        "WHY":(
            f"{efit.get('reason','')}; {vfit.get('reason','')}; "
            f"{m.get('price_confidence_grade','')} pricing; {int(m.get('comp_count',0) or 0)} matching comps; "
            f"{m.get('pricing_method','')}"
        ),
    }
...
def all_opportunities(inv,allow_unpriced=False):
    ...
    expected_cols=[
        ...
        "CONF","FITMENTS","RISK_DISC","DWMT_SCORE","DWMT","COMP_COUNT","PRICE_SPREAD_PCT","VALUE_STABILITY",
        "RECOMMENDABLE","QUICK_PULL_REASON","VIN_FIT","VIN_REASON","TRIM","SERIES",
        "ENGINE","FUEL","TURBO","ENGINE_FIT","ENGINE_REASON",
        "WEIGHT","LENGTH","WIDTH","HEIGHT","WARNING","LEARNED_WARNING",
        "MARKET_DATA","MARKET_SOURCES","PRICE_SOURCE_URL","PRICE_SOURCE_TITLE","PRICING_METHOD","PRICE_CONFIDENCE","WALKAWAY_REASON","WHY"
    ]
...
def _ranking_settings_signature():
    return (
        ...
        bool(st.session_state.get("strict_vin_fitment",True)),
        int(float(st.session_state.get("_vin_decode_cache_ts",0) or 0)//900),
        bool(st.session_state.get("quick_pull_only",True)),
        int(st.session_state.get("max_pull_minutes",60)),
        round(float(st.session_state.get("max_part_weight",80)),1),
        round(float(st.session_state.get("max_part_dimension",48)),1),
        round(float(st.session_state.get("max_ship_estimate",75)),1),
...
def get_opportunities_cached(inv,allow_unpriced=False):
    ...
    """
    # Pull the background scanner's persistent cloud prices into the UI.
    # This is what makes BEST CARS / TODAY'S PLAN use the completed whole-yard scan.
    if not allow_unpriced:
        refresh_cloud_market_cache()
    ...
    if key not in cache:
        ...
        if (not allow_unpriced) and bool(st.session_state.get("market_only",True)):
            cache[key]=market_priced_opportunities_fast(inv)
        else:
...
def build_today_plan(opp,budget,hours,max_cars=8,parts_per_car=4,min_conf=35,min_presence=25,min_profit=75,min_dwmt=50):
    """
    ...
    while len(remaining):
        ...
        for idx,r in remaining.head(180).iterrows():
            ...
            dwmt=float(r.get("DWMT_SCORE",50))/100
            utility=(exp_profit*reuse_bonus*(.62+.15*conf+.08*presence+.15*dwmt)) / max(.18+cost_pressure+time_pressure, .05)
...
def market_entry_has_value(entry):
    """True when a cached market entry contains a usable price from any live source."""
    ...
    for provider in ("ebay","shopping"):
        ...
        if isinstance(p,dict):
            med=p.get("median")
            try:
                if med is not None and pd.notna(med) and float(med)>0:
                    return True
            except Exception:
                pass
    return False

def recompute_market_covered_count_fast():
    """
    Cheap coverage rebuild from inventory + market_cache only.

    This avoids a full opportunity rescore and fixes the header after:
    - app restart
    - Supabase restore
    - local-memory restore
    - inventory sync

    A cached search applies to every matching year/make/model donor when OEM
    number is blank, which is already how market_key() shares search results.
    """
    inv=st.session_state.get("inventory")
    cache=st.session_state.get("market_cache",{})

    if inv is None or not isinstance(inv,pd.DataFrame) or len(inv)==0 or not cache:
        st.session_state.market_covered_count=0
        return 0

    # Prefixes with at least one real cached market value.
    valid_prefixes=set()
    for key,entry in cache.items():
        if not market_entry_has_value(entry):
            continue
        parts=str(key).split("|")
        if len(parts)>=4:
            # year|make|model|part|pn  -> donor coverage only needs Y/M/M
            valid_prefixes.add("|".join(parts[:3]).lower().strip())
    ...
    for _,r in inv.iterrows():
        prefix=f"{r.get('year','')}|{r.get('make','')}|{r.get('model','')}".lower().strip()
        if prefix in valid_prefixes:
            covered+=1

    st.session_state.market_covered_count=int(covered)
    return int(covered)


# ---------------- FAST YARD CANDIDATES ----------------
def fast_research_candidates(inv):
    """
    Cheap candidate generation for MARKET.

    It intentionally does NOT call part_market(), sold-comps matching, fitment
    matching, channel analysis, etc. Those expensive operations are only run
    after a candidate actually has market data or the user inspects one donor.
    """
    expected=[
        "vehicle_id","CAR","STOCK","VIN","SECTION","ROW","SPACE","YARD_LOCATION",
        "AVAILABLE","DAYS_YARD","PART","BUY","MIN","SCORE","EXPECTED_PROFIT","PRESENT_PCT","RECOMMENDABLE","QUICK_PULL_REASON","VIN_FIT","VIN_REASON","TRIM","SERIES","ENGINE","FUEL","TURBO","ENGINE_FIT","ENGINE_REASON"
    ]
    if inv is None or len(inv)==0:
        return pd.DataFrame(columns=expected)

    avoid_norm={norm(x) for x in st.session_state.get("avoid_parts",set())}
    rows=[]
    part_records=PARTS.to_dict("records")

    # itertuples is noticeably faster than nested DataFrame.iterrows on 1,700+ cars.
    for idx,v in inv.iterrows():
        vd=v.to_dict()
        car=f"{vd.get('year')} {vd.get('make')} {vd.get('model')}"
        for pr in part_records:
            p=type("P",(),pr)
            app=applicability(vd,p)
            if app<.10:
                continue
            efit=engine_part_fitment(vd,pr["part"])
            if pr["part"] in ENGINE_SENSITIVE_PARTS and not efit["allowed"]:
                continue
            if norm(pr["part"]) in avoid_norm:
                continue

            quick_ok,quick_reason=quick_pull_from_part_row(p)
            # Normal MARKET scan spends API calls only on parts the app is
            # willing to recommend. The full EBAY SWEEP may still scan all rows.
            if bool(st.session_state.get("quick_pull_only",True)) and not quick_ok:
                continue
            vfit=vin_part_fitment(vd,pr["part"])
            if bool(st.session_state.get("strict_vin_fitment",True)) and not vfit["allowed"]:
                continue

            market=fallback_market(vd,p)
            buy=get_yard_cost(p)
            rough_profit=max(0.0,market*.82-buy-float(st.session_state.get("travel",10)))
            present,age=presence_probability(vd,p)
            expected_profit=rough_profit*present
            pph=rough_profit/max(float(pr["pull_minutes"])/60,.1)
            score=(
                min(expected_profit/350*38,38)+
                min(pph/150*25,25)+
                float(pr["demand_score"])*20+
                app*10+
                present*7
            )
            rows.append({
                ...
                "YARD_LOCATION":vd.get("yard_location","") or "Location pending",
                "AVAILABLE":vd.get("available_date",""),
                "DAYS_YARD":None if age==999 else age,
                "PART":pr["part"],
                "BUY":round(float(buy),2),
                "MIN":int(pr["pull_minutes"]),
                "SCORE":round(float(min(score,100)),1),
                "EXPECTED_PROFIT":round(float(expected_profit),2),
                "PRESENT_PCT":round(float(present)*100,0),
                "RECOMMENDABLE":bool(quick_ok),"QUICK_PULL_REASON":quick_reason,
                "VIN_FIT":vfit.get("status",""),"VIN_REASON":vfit.get("reason",""),
                "TRIM":vfit.get("trim",""),"SERIES":vfit.get("series",""),
                "ENGINE":engine_display(vd),"FUEL":engine_profile(vd).get("fuel",""),
                "TURBO":engine_profile(vd).get("turbo",""),
                "ENGINE_FIT":efit.get("status",""),"ENGINE_REASON":efit.get("reason",""),
            })
    return pd.DataFrame(rows,columns=expected)

def get_fast_research_candidates(inv):
    """
    Session cache for the cheap whole-yard research list.
    Market-price updates do not change this list, so it survives market scans.
    """
    cache=st.session_state.setdefault("_fast_candidate_cache",{})
    key=(
        str(st.session_state.get("last_sync","")),
        len(inv) if inv is not None else 0,
        tuple(sorted(str(x) for x in st.session_state.get("avoid_parts",set()))),
        round(float(st.session_state.get("travel",10)),2),
        bool(st.session_state.get("strict_vin_fitment",True)),
        int(float(st.session_state.get("_vin_decode_cache_ts",0) or 0)//900),
        bool(st.session_state.get("quick_pull_only",True)),
        int(st.session_state.get("max_pull_minutes",60)),
        ...
        round(float(st.session_state.get("max_part_dimension",48)),1),
        round(float(st.session_state.get("max_ship_estimate",75)),1),
        str(st.session_state.get("lkq_price_source","")),
    )
    if key not in cache:
        cache.clear()
        cache[key]=fast_research_candidates(inv)
    return cache[key]

def market_priced_opportunities_fast(inv):
    """
    BEST CARS fast path.

    With market-only mode enabled and no imported sold-comps table, only
    vehicle/part combinations whose shared market key actually has a usable
    live price can become recommendations. Find those cheaply first, then run
    the full opportunity model on that much smaller set.
    """
    if inv is None or len(inv)==0:
        return all_opportunities(inv,allow_unpriced=False)

    # Imported sold comps can create market data without market_cache entries.
    # Fall back to the comprehensive path in that uncommon case.
    if isinstance(st.session_state.get("sold_comps"),pd.DataFrame) and len(st.session_state.sold_comps):
        return all_opportunities(inv,allow_unpriced=False)

    research=get_fast_research_candidates(inv)
    if not len(research):
        return all_opportunities(inv.iloc[0:0],allow_unpriced=False)

    valid_cache={
        k for k,v in st.session_state.get("market_cache",{}).items()
        if market_entry_has_value(v)
    }
    if not valid_cache:
        return all_opportunities(inv.iloc[0:0],allow_unpriced=False)

    rows=[]
    pmap={r["part"]:r for r in PARTS.to_dict("records")}
    for _,r in research.iterrows():
        pr=pmap.get(r["PART"])
        if not pr:
            continue
        vd=inv.loc[r["vehicle_id"]].to_dict()
        pn=part_number(r["STOCK"],r["PART"])
        key=market_key(vd,r["PART"],pn)
        # Generic market lookup if no OEM number, exact if OEM was scanned.
        if key not in valid_cache:
            generic_key=market_key(vd,r["PART"],"")
            if generic_key not in valid_cache:
                continue
        p=type("P",(),pr)
        o=opportunity(vd,p,allow_unpriced=False)
        if not o:
            continue
        rows.append({
            "vehicle_id":r["vehicle_id"],"CAR":r["CAR"],"STOCK":r["STOCK"],"VIN":r["VIN"],
    ...
    if not rows:
        return all_opportunities(inv.iloc[0:0],allow_unpriced=False)
    expected=all_opportunities(inv.iloc[0:0],allow_unpriced=False).columns.tolist()
    return pd.DataFrame(rows).reindex(columns=expected)
...
def whole_yard_coverage(base=None):
    """
    Fast donor coverage derived from market_cache rather than a full scored-yard
    DataFrame. This keeps the MARKET page responsive.
    """
    inv=st.session_state.get("inventory")
    total=len(inv) if isinstance(inv,pd.DataFrame) else 0
    if total==0:
        return {"total":0,"covered_ids":set(),"covered":0,"remaining":0,"exhausted_ids":set(),"exhausted":0}

    valid_prefixes=set()
    for key,entry in st.session_state.get("market_cache",{}).items():
        if not market_entry_has_value(entry):
            continue
        parts=str(key).split("|")
        if len(parts)>=3:
    ...
    exhausted_ids=set()
    if base is not None and isinstance(base,pd.DataFrame) and len(base):
        # Only inspect the small top candidate set per uncovered donor.
        for vid,g in base.groupby("vehicle_id",sort=False):
            if vid in covered_ids:
                continue
            plausible=g.nlargest(min(4,len(g)),"SCORE")
    ...
    return {
        "total":total,"covered_ids":covered_ids,"covered":len(covered_ids),
        "remaining":max(0,total-len(covered_ids)),
        "exhausted_ids":exhausted_ids,"exhausted":len(exhausted_ids),
    }
...
def whole_yard_candidate_order(base,parts_per_car=3,only_uncovered=True):
    ...
    for vid,g in base.groupby("vehicle_id"):
        ...
        days=_safe_float(g.iloc[0].get("DAYS_YARD"),30)
        freshness=max(0.0,20.0-min(days,30.0)/30.0*20.0)
        donor_rank.append((vid,float(g.iloc[0]["SCORE"])+freshness,float(g.iloc[0]["EXPECTED_PROFIT"])))

    # V10: fresh/new arrivals get a priority bump, then round-robin fairness.
    ...
    if cid and secret:
        if _provider_fresh(entry,"ebay",ttl_hours) and entry.get("ebay"):
            ...
            try:
                ...
                df=filter_comps_for_engine(df,vehicle,part)
                summ=summarize_comps(df)
                if len(df) and pd.notna(summ.get("median",np.nan)):
                    ex=df.assign(_d=(pd.to_numeric(df["landed"],errors="coerce")-float(summ["median"])).abs()).sort_values("_d").head(4).drop(columns=["_d"]).to_dict("records")
                else:
                    ex=[]
                entry["ebay"]={**summ,"examples":ex,"pricing_method":"engine-checked conservative active comp"}
        ...
        if _provider_fresh(entry,"shopping",ttl_hours) and entry.get("shopping"):
            ...
            try:
                ...
                if len(df) and pd.notna(summ.get("median",np.nan)):
                    ...
                    ex=[]
                entry["shopping"]={**summ,"examples":ex,"pricing_method":"engine-checked conservative shopping comp"}
...
def price_candidates(cands,max_queries,progress=None,status=None):
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(future_map):
            try:
                ...
                _append_price_history(key,entry)
                calls=st.session_state.setdefault("api_calls_total",{"ebay":0,"shopping":0})
                for provider in fetched:
                    pk="ebay" if provider=="eBay" else "shopping"
                    calls[pk]=int(calls.get(pk,0))+1
    ...
    if fetched_queries:
        ...
        recompute_market_covered_count_fast()
        try:
            refresh_precomputed_recommendations()
        except Exception:
            pass
...
def backup_payload():
    return json_safe({
        ...
        "quick_pull_only":bool(st.session_state.get("quick_pull_only",True)),
        "strict_vin_fitment":bool(st.session_state.get("strict_vin_fitment",True)),
        "max_pull_minutes":int(st.session_state.get("max_pull_minutes",60)),
        "max_part_weight":float(st.session_state.get("max_part_weight",80)),
        "max_part_dimension":float(st.session_state.get("max_part_dimension",48)),
        "max_ship_estimate":float(st.session_state.get("max_ship_estimate",75)),
        "price_history":st.session_state.get("price_history",[]),
        "api_calls_total":st.session_state.get("api_calls_total",{"ebay":0,"shopping":0}),
        "precomputed_recommendations":st.session_state.get("precomputed_recommendations",[]),
        "precomputed_at":st.session_state.get("precomputed_at","Never"),
        "alert_min_part_profit":float(st.session_state.get("alert_min_part_profit",150)),
        "alert_min_donor_profit":float(st.session_state.get("alert_min_donor_profit",400)),
        "alert_min_dwmt":int(st.session_state.get("alert_min_dwmt",65)),
        "catalog_sweep_signature":st.session_state.get("catalog_sweep_signature",""),
        "catalog_sweep_cursor":int(st.session_state.get("catalog_sweep_cursor",0)),
        "catalog_sweep_results":st.session_state.get("catalog_sweep_results",[]),
        "catalog_sweep_errors":int(st.session_state.get("catalog_sweep_errors",0)),
        "catalog_sweep_finished":bool(st.session_state.get("catalog_sweep_finished",False)),
...
def restore_payload(data):
    ...
    st.session_state.quick_pull_only=bool(data.get("quick_pull_only",st.session_state.get("quick_pull_only",True)))
    st.session_state.strict_vin_fitment=bool(data.get("strict_vin_fitment",st.session_state.get("strict_vin_fitment",True)))
    st.session_state.max_pull_minutes=int(data.get("max_pull_minutes",st.session_state.get("max_pull_minutes",60)))
    st.session_state.max_part_weight=float(data.get("max_part_weight",st.session_state.get("max_part_weight",80)))
    st.session_state.max_part_dimension=float(data.get("max_part_dimension",st.session_state.get("max_part_dimension",48)))
    st.session_state.max_ship_estimate=float(data.get("max_ship_estimate",st.session_state.get("max_ship_estimate",75)))
    st.session_state.price_history=data.get("price_history",st.session_state.get("price_history",[]))
    st.session_state.api_calls_total=data.get("api_calls_total",st.session_state.get("api_calls_total",{"ebay":0,"shopping":0}))
    st.session_state.precomputed_recommendations=data.get("precomputed_recommendations",st.session_state.get("precomputed_recommendations",[]))
    st.session_state.precomputed_at=data.get("precomputed_at",st.session_state.get("precomputed_at","Never"))
    st.session_state.alert_min_part_profit=float(data.get("alert_min_part_profit",st.session_state.get("alert_min_part_profit",150)))
    st.session_state.alert_min_donor_profit=float(data.get("alert_min_donor_profit",st.session_state.get("alert_min_donor_profit",400)))
    st.session_state.alert_min_dwmt=int(data.get("alert_min_dwmt",st.session_state.get("alert_min_dwmt",65)))
    st.session_state.catalog_sweep_signature=data.get("catalog_sweep_signature","")
    st.session_state.catalog_sweep_cursor=int(data.get("catalog_sweep_cursor",0))
    st.session_state.catalog_sweep_results=data.get("catalog_sweep_results",[])
    st.session_state.catalog_sweep_errors=int(data.get("catalog_sweep_errors",0))
    st.session_state.catalog_sweep_finished=bool(data.get("catalog_sweep_finished",False))
    recompute_market_covered_count_fast()
...
def _local_memory_path():
    ...
    return Path("/tmp")/f"{mem_id}_scanner_memory.pkl"

def _old_local_json_path():
    mem_id=re.sub(r"[^A-Za-z0-9_.-]","_",str(st.session_state.get("memory_id","wichita-parts")))
    return Path("/tmp")/f"{mem_id}_scanner_memory.json"

def save_local_memory():
    """Binary local persistence is much faster than encoding the whole state to JSON."""
    path=_local_memory_path()
    with path.open("wb") as f:
        pickle.dump(backup_payload(),f,protocol=pickle.HIGHEST_PROTOCOL)
    return str(path)
...
def load_local_memory():
    ...
    if path.exists():
        with path.open("rb") as f:
            restore_payload(pickle.load(f))
        return True,f"Loaded fast local memory from {path.name}."

    # One-time backward compatibility with V8/V9 JSON local memory.
    old=_old_local_json_path()
    if old.exists():
        restore_payload(json.loads(old.read_text()))
        return True,f"Loaded older local memory from {old.name}."
...
def autosave_memory_if_changed():
    """
    Fast local autosave only.

    Page-to-page navigation must never wait on Supabase. Cloud saves are done
    explicitly in SETUP -> Memory or after long-running market jobs where the
    user has already been waiting for network work.
    """
    if not st.session_state.get("memory_autosave",True):
        return
    if not st.session_state.get("_memory_dirty",False):
        return

    now=time.time()
    if (now-float(st.session_state.get("_last_mutation_ts",0))) < .75:
        return
    try:
        save_local_memory()
        st.session_state["_memory_dirty"]=False
        st.session_state.memory_status="Saved locally (fast). Cloud save pending."
    except Exception as e:
...
defaults={
    ...
    "catalog_sweep_signature":"","catalog_sweep_cursor":0,
    "catalog_sweep_results":[],"catalog_sweep_errors":0,"catalog_sweep_finished":False,
    "ranking_revision":0,"opportunity_cache":{},"_fast_candidate_cache":{},
    ...
    "plan_min_conf":35,"plan_min_presence":25,"plan_min_profit":75,"plan_min_dwmt":55,
    "today_plan":pd.DataFrame(),
    "price_history":[],"api_calls_total":{"ebay":0,"shopping":0},
    "precomputed_recommendations":[],"precomputed_at":"Never",
    "alert_min_part_profit":150.0,"alert_min_donor_profit":400.0,"alert_min_dwmt":65,
    ...
    "supabase_connected":False,"supabase_connection_status":"Not tested yet.","_supabase_probe_done":False,"_cloud_restore_available":False,
    "ebay_fee":.13,"local_fee":0.0,"ebay_quick_pct":.88,"local_price_pct":.82,
    "travel":10.0,"packing":5.0,"high_value_alert":600.0,
    "quick_pull_only":True,"strict_vin_fitment":True,
    "_vin_decode_cache":{},"_vin_decode_cache_ts":0.0,"_vin_decode_status":"Not loaded",
    "max_pull_minutes":60,"max_part_weight":80.0,
    "max_part_dimension":48.0,"max_ship_estimate":75.0,
}
...
# Fast startup:
# Never make a network request before the first screen renders.
# Local /tmp memory is loaded immediately. If this is a true cold Streamlit
# container and local memory is gone, the UI shows a one-tap cloud restore.
if not st.session_state.get("memory_loaded",False):
    ...
    st.session_state.memory_status=msg or (
        "Fast boot complete. Restore Supabase memory if this is a cold restart."
        if cloud_memory_enabled() else
        "No saved local memory found."
    )
    st.session_state["_cloud_restore_available"]=bool((not loaded) and cloud_memory_enabled())
    ensure_df("purchases",PURCHASE_COLS)
    ensure_df("listings",LISTING_COLS)
    ensure_df("sales",SALE_COLS)
    recompute_market_covered_count_fast()
...
_cloud_run=refresh_cloud_run_summary()
_cloud_total=int(_safe_float((_cloud_run or {}).get("market_total"),0))
_cloud_scanned=int(_safe_float((_cloud_run or {}).get("market_scanned"),0))
_cloud_priced=int(_safe_float((_cloud_run or {}).get("market_priced"),0))
_cloud_pct=_safe_float((_cloud_run or {}).get("market_coverage_pct"),0)
_market_chip=(
    f"☁️ {_cloud_priced:,}/{_cloud_total:,} priced market combos"
    if _cloud_total else
    f"⚡ {len(st.session_state.market_cache):,} local cached searches"
)
_coverage_chip=(
    f"🎯 {_cloud_pct:.0f}% cloud yard sweep"
    if _cloud_total else
    f"🎯 {min(int(st.session_state.get('market_covered_count',0)),len(st.session_state.inventory))}/{len(st.session_state.inventory)} locally covered cars"
)
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V10.5</span></div>
      ...
      <div class="chips">
        ...
        <span class="chip">{_market_chip}</span>
        <span class="chip">{_coverage_chip}</span>
        <span class="chip">🧠 {_cloud}</span>
        <span class="chip">🤖 cloud autopilot ready</span>
      </div>
    </div>""",
    unsafe_allow_html=True
)

if st.session_state.get("_cloud_restore_available",False):
    c_restore,c_skip=st.columns([3,1])
    if c_restore.button("☁️ RESTORE MY SUPABASE MEMORY",type="primary"):
        try:
            ok,msg=load_cloud_memory()
            if ok:
                st.session_state["_cloud_restore_available"]=False
            ...
            st.error(str(e))
    if c_skip.button("Skip"):
        st.session_state["_cloud_restore_available"]=False

MAIN_PAGES=[
    "⚡ TODAY'S PLAN","🏆 BEST CARS","🧭 YARD MODE","🔄 SYNC","📷 SCAN",
    "🌐 MARKET","🧠 DISCOVERY","🧮 EBAY SWEEP","🤖 AUTOPILOT","📦 BUSINESS","📊 DASHBOARD","⚙️ SETUP"
]
page=st.radio(
    "Page",
    MAIN_PAGES,
...
if page=="⚡ TODAY'S PLAN":
    ...
    if not len(st.session_state.inventory):
        ...
        with st.form("today_plan_form"):
            ...
            c7,c8=st.columns(2)
            min_profit=c7.number_input("Minimum profit per part",0,1000,int(st.session_state.plan_min_profit),25)
            min_dwmt=c8.slider("Minimum Don't Waste My Time score",0,100,int(st.session_state.plan_min_dwmt),5)
        ...
        if submitted:
            ...
            st.session_state.plan_min_dwmt=min_dwmt
            st.session_state.today_plan=build_today_plan(
                opp,budget,hours,max_cars,parts_per_car,min_conf,min_presence,min_profit,min_dwmt
            )
            mark_memory_dirty("today plan")
            if st.session_state.today_plan is not None and len(st.session_state.today_plan):
                st.success(
                    f"Built a plan with {len(st.session_state.today_plan)} pull(s) from "
                    f"{st.session_state.today_plan['STOCK'].astype(str).nunique()} donor car(s)."
                )
            else:
                st.warning(
                    f"No parts met all of those filters. The app currently has {len(opp):,} "
                    f"market-priced opportunity row(s) before the plan filters. Try lowering minimum profit, "
                    f"DWMT or confidence."
                )
            ...
            for _,car in summary.iterrows():
                ...
                for _,r in parts.iterrows():
                    ph+=(
                        ...
                        f"<span class='small'>{r['DWMT']} {r['DWMT_SCORE']:.0f}/100 · {r.get('VIN_FIT','VIN fit —')} · {r['PRESENT_PCT']:.0f}% likely present · "
...
if page=="🏆 BEST CARS":
    ...
    if not len(st.session_state.inventory):
        ...
        opp=get_opportunities_cached(st.session_state.inventory)
        opp=filter_recommendable(opp)
        good=opp[(opp["PROFIT"]>=min_profit)&(opp["SCORE"]>=min_score)].copy()
        good=good.sort_values(["DWMT_SCORE","EXPECTED_PROFIT","SCORE"],ascending=False)

        if not len(good):
            cloud=refresh_cloud_run_summary()
            cloud_priced=int(_safe_float((cloud or {}).get("market_priced"),0))
            if cloud_priced>0 and len(opp):
                st.warning(
                    f"Cloud pricing is loaded ({cloud_priced:,} priced combinations), but nothing meets the "
                    f"current Minimum profit / Minimum score filters plus Quick-Pull and fitment rules. "
                    f"Lower the filters above to see more cars."
                )
            elif cloud_priced>0:
                st.warning(
                    f"The cloud worker has {cloud_priced:,} priced combinations, but none currently pass the "
                    f"app's Quick-Pull / VIN / engine rules. Refresh the page once; if this remains empty, "
                    f"lower the filters or check DISCOVERY."
                )
            else:
                st.info("No cloud market pricing is available yet. Check MARKET / AUTOPILOT for the latest worker run.")
        else:
            donors=[]
            for vid,g in good.groupby("vehicle_id"):
                top=g.nlargest(5,"DWMT_SCORE")
                f=top.iloc[0]
                donors.append({
                    ...
                    "TOP5_PROFIT":top["PROFIT"].sum(),"BEST_SCORE":top["SCORE"].max(),"BEST_DWMT":top["DWMT_SCORE"].max(),
                    "AVG_DAYS":top["DAYS"].mean()
                })
            donors=pd.DataFrame(donors).sort_values(["BEST_DWMT","TOP5_PROFIT","BEST_SCORE"],ascending=False)
            ...
            st.caption("Recommendations are market-driven and Quick-Pull filtered. Complete engines/transmissions and bulky/slow/expensive-to-ship parts are hidden by default.")

            for _,drow in donors.head(show_cards).iterrows():
                g=good[good["vehicle_id"]==drow.vehicle_id].nlargest(5,"DWMT_SCORE")
                ph=""
                for _,r in g.iterrows():
                    warn=""
                    ...
                    ph+=(
                        ...
                        f"<span class='small'><b>{r['DWMT']} {r['DWMT_SCORE']:.0f}/100</b> · {r['REC']} · Market {money(r['MARKET'])} · "
                        f"{r['PRESENT_PCT']:.0f}% likely present · ~{r['DAYS']} days to sell · "
                        f"Confidence {r['CONF']}% · {r['VALUE_SOURCE']}</span>"
                        + (f"<br><a href='{r.get('PRICE_SOURCE_URL','')}' target='_blank'>🔗 Open price comp</a>" if r.get("PRICE_SOURCE_URL","") else "")
                        + f"{warn}</div>"
                    )
                st.markdown(
            ...
            if st.toggle("Show full opportunity table",value=False,key="show_full_best_table"):
                cols=["NEW","CAR","YARD_LOCATION","AVAILABLE","DAYS_YARD","ENGINE","FUEL","PART","ENGINE_FIT","DWMT","DWMT_SCORE","REC","BUY","MARKET","TARGET","CHANNEL","PROFIT","EXPECTED_PROFIT","PRESENT_PCT","$/HR","DAYS","CONF","PRICE_CONFIDENCE","SCORE","VALUE_SOURCE","PRICE_SOURCE_URL","WHY","STOCK"]
                st.dataframe(
                    good[cols],use_container_width=True,hide_index=True,
                    column_config={"PRICE_SOURCE_URL":st.column_config.LinkColumn("Price source",display_text="Open comp")}
                )
...
if page=="🧭 YARD MODE":
    ...
    if not len(st.session_state.inventory):
        ...
        opp=filter_recommendable(get_opportunities_cached(st.session_state.inventory)).sort_values(["SCORE","PROFIT"],ascending=False)
...
if page=="🔄 SYNC":
    ...
    if st.button("🔄 SYNC WHOLE WICHITA YARD",type="primary"):
        try:
            ...
            if len(df):
                ...
                st.session_state.catalog_sweep_signature=""
                st.session_state.catalog_sweep_finished=False
                st.session_state["_fast_candidate_cache"]={}
                recompute_market_covered_count_fast()
                invalidate_rankings()
    ...
    if len(st.session_state.inventory):
        ...
        vcache=refresh_cloud_vin_cache()
        inv_show["vin_trim"]=inv_show.get("vin",pd.Series(index=inv_show.index,dtype=str)).astype(str).str.upper().map(lambda v:(vcache.get(v) or {}).get("trim", ""))
        inv_show["vin_series"]=inv_show.get("vin",pd.Series(index=inv_show.index,dtype=str)).astype(str).str.upper().map(lambda v:(vcache.get(v) or {}).get("series", ""))
        show_cols=[c for c in [
            "year","make","model","vin_trim","vin_series","color","yard_location","available_date","days_in_yard",
...
if page=="🌐 MARKET":
    ...
    cloud_hydrate=refresh_cloud_market_cache()

    cloud_runs,cloud_run_status=background_rest_rows("scanner_runs",{
        "select":"started_at,status,cars_found,new_cars,ebay_calls,market_total,market_scanned,market_priced,market_coverage_pct,error",
        "order":"started_at.desc","limit":"1"
    },1)
    cloud_latest=cloud_runs[0] if cloud_runs else {}
    if cloud_latest:
        cloud_pct=_safe_float(cloud_latest.get("market_coverage_pct"),0)
        cloud_scanned=int(_safe_float(cloud_latest.get("market_scanned"),0))
        cloud_total=int(_safe_float(cloud_latest.get("market_total"),0))
        cloud_priced=int(_safe_float(cloud_latest.get("market_priced"),0))
        st.info(
            f"☁️ Cloud whole-yard scan: {cloud_scanned:,}/{cloud_total:,} quick-pull market combinations "
            f"({cloud_pct:.1f}%) attempted · {cloud_priced:,} have usable eBay pricing. "
            f"The background worker re-syncs the yard twice daily and prices new arrivals first."
        )
        if cloud_hydrate.get("rows",0):
            st.caption(
                f"Loaded {cloud_hydrate['rows']:,} cloud market rows into this app session "
                f"({cloud_hydrate.get('priced',0):,} with usable prices), so BEST CARS and TODAY'S PLAN use the background scan."
            )
    ...
    if not len(st.session_state.inventory):
        ...
        covered=min(int(st.session_state.get("market_covered_count",0)),total)
        pct=(100*covered/total) if total else 0

        a,b,c,d=st.columns(4)
        a.metric("Yard cars",total)
        b.metric("Cars with market price",covered)
        c.metric("Coverage",f"{pct:.0f}%")
        d.metric("Cached searches",len(st.session_state.market_cache))

        # THE SCAN BUTTON NOW RENDERS BEFORE ANY WHOLE-YARD SCORING.
        with st.form("whole_yard_market_scan_v91"):
            c1,c2,c3=st.columns(3)
            scope=c1.selectbox(
                "Scan scope",
                ["🎯 Cover unpriced cars first","🔬 Deepen all cars"],
                help="Cover mode gives each donor a chance before doing extra searches on already-priced cars."
            )
            batch=c2.slider("Unique candidates this run",5,80,20,5)
            parts_per_car=c3.slider("Candidate parts/car",1,5,3,1)
            run_whole=st.form_submit_button("🔎 SCAN MARKET NOW",type="primary")

        st.progress(min(1.0,pct/100 if total else 0))
        st.caption(
            "This button is an optional on-demand local scan. V10.4 also runs a persistent cloud whole-yard sweep in the background, saving progress to Supabase and prioritizing new arrivals after each twice-daily yard sync."
        )
        ...
        if run_whole:
            ...
            stat=st.empty()
            try:
                # Cheap research ranking instead of full 40k+ opportunity model.
                base=get_fast_research_candidates(st.session_state.inventory)
                ...
                st.success(
                    f"Finished: {donors_touched} donor car(s) targeted · "
                    f"{fetched} internet refresh(es) · {skipped} cached/shared · "
                    f"coverage {new_cov['covered']}/{new_cov['total']} ({pct2:.0f}%)."
                )
                if errors:
        ...
        # These tools are optional and do not block the scan button.
        with st.expander("Market options / imports"):
        ...
        if st.toggle("Show cars still needing a market price",value=False,key="market_remaining_v91"):
            valid_prefixes=set()
            for key,entry in st.session_state.market_cache.items():
                if market_entry_has_value(entry):
                    parts=str(key).split("|")
                    if len(parts)>=3:
                        valid_prefixes.add("|".join(parts[:3]).lower().strip())
            inv=st.session_state.inventory
            ...
            for _,r in inv.iterrows():
                prefix=f"{r.get('year','')}|{r.get('make','')}|{r.get('model','')}".lower().strip()
                mask.append(prefix not in valid_prefixes)
            remaining=inv[pd.Series(mask,index=inv.index)]
        ...
        if st.button("SHOW THIS DONOR'S PARTS"):
            # Score only ONE selected donor, not all 1,722 cars.
            caropp=all_opportunities(
                st.session_state.inventory.loc[[vi]],
                allow_unpriced=True
            ).sort_values(["MARKET_DATA","SCORE","PROFIT"],ascending=False)
            st.session_state["_market_inspect_rows"]=caropp.to_dict("records")

        inspect=pd.DataFrame(st.session_state.get("_market_inspect_rows",[]))
        if len(inspect):
            cols=[c for c in ["PART","ENGINE","FUEL","TURBO","ENGINE_FIT","ENGINE_REASON","TRIM","SERIES","VIN_FIT","VIN_REASON","DWMT","DWMT_SCORE","MARKET_DATA","MARKET","TARGET","PROFIT","EXPECTED_PROFIT","CONF","COMP_COUNT","PRICE_SPREAD_PCT","VALUE_STABILITY","PRESENT_PCT","SCORE","VALUE_SOURCE"] if c in inspect.columns]
            st.dataframe(inspect[cols].head(20),use_container_width=True,hide_index=True)

    if st.session_state.market_cache and st.toggle("Show cached market references",value=False,key="show_market_cache_v91"):
        rows=[]
        ...
        st.dataframe(pd.DataFrame(rows).head(500),use_container_width=True,hide_index=True)

# ---------- V10.4 CAR-FIRST DISCOVERY ----------
if page=="🧠 DISCOVERY":
    st.subheader("Car-first part discovery")
    st.caption(
        "V10.5 searches eBay from the car outward instead of only testing the built-in parts catalog. "
        "It clusters the parts appearing for each exact VIN/engine/trim family, estimates LKQ cost/pull time, "
        "and ranks compact high-profit opportunities. Sold-comps are used when available; otherwise demand is labeled as an active-market proxy."
    )

    runs,_=background_rest_rows("scanner_runs",{
        "select":"started_at,status,cars_found,new_cars,ebay_calls,discovery_families_scanned,discovery_parts_found,discovery_recommendations,discovery_calls,error",
        "order":"started_at.desc","limit":"1"
    },1)
    latest=runs[0] if runs else {}
    fams,_=background_rest_rows("scanner_discovery_families",{
        "select":"family_id,status,last_scanned,listing_count,discovered_count,error","order":"last_scanned.desc","limit":"1000"
    },1000)
    recs,_=background_rest_rows("scanner_discovered_recommendations",{
        "select":"*","order":"dwmt_score.desc,expected_net.desc","limit":"1000"
    },1000)
    discovered_parts,_=background_rest_rows("scanner_discovered_parts",{
        "select":"*","order":"dwmt_score.desc,expected_net.desc","limit":"3000"
    },3000)

    a,b,c,d=st.columns(4)
    a.metric("Families scanned",f"{len(fams):,}" if fams else "0")
    b.metric("Last-run discoveries",f"{int(_safe_float(latest.get('discovery_parts_found'),0)):,}")
    c.metric("Live discovery pulls",f"{len(recs):,}")
    d.metric("Discovery eBay calls",f"{int(_safe_float(latest.get('discovery_calls'),0)):,}")

    if latest:
        st.info(
            f"Last background run: {latest.get('started_at','—')} · "
            f"{int(_safe_float(latest.get('discovery_families_scanned'),0))} exact donor families scanned · "
            f"{int(_safe_float(latest.get('discovery_parts_found'),0))} part clusters found · "
            f"{int(_safe_float(latest.get('discovery_recommendations'),0))} donor-specific recommendations materialized."
        )

    if recs:
        recs=[r for r in recs if not discovery_title_hard_blocked(r.get("part_name",""))]
        rdf=pd.DataFrame(recs)
        with st.form("discovery_filters"):
            c1,c2,c3=st.columns(3)
            min_net=c1.number_input("Minimum expected net",0,2000,100,25)
            min_dwmt=c2.slider("Minimum DWMT",0,100,55,5)
            sold_only=c3.toggle("Require actual sold-comp evidence",value=False)
            go=st.form_submit_button("APPLY DISCOVERY FILTERS",type="primary")
        show=rdf.copy()
        if "expected_net" in show: show=show[pd.to_numeric(show.expected_net,errors="coerce").fillna(0)>=min_net]
        if "dwmt_score" in show: show=show[pd.to_numeric(show.dwmt_score,errors="coerce").fillna(0)>=min_dwmt]
        if sold_only and "sold_count_90" in show:
            show=show[pd.to_numeric(show.sold_count_90,errors="coerce").fillna(0)>0]
        sort_cols=[c for c in ["dwmt_score","expected_net","profit_per_hour"] if c in show.columns]
        if sort_cols: show=show.sort_values(sort_cols,ascending=False)
        cols=[c for c in [
            "car","trim","engine_label","yard_location","part_name","fitment_status","market_basis",
            "sold_count_90","active_count","sell_through_pct","saturation_status","market_value","lkq_cost","expected_net",
            "profit_per_hour","pull_minutes","dwmt_score","confidence","price_confidence_grade","oem_match_status","price_source_url","stock_number"
        ] if c in show.columns]
        st.dataframe(
            show[cols].head(300),use_container_width=True,hide_index=True,
            column_config={"price_source_url":st.column_config.LinkColumn("Price source",display_text="Open comp")}
        )
    else:
        st.info("Discovery rows will appear after the V10.4 background workflow completes its first run.")

    st.markdown("### Exact donor inspector")
    if not len(st.session_state.inventory):
        st.info("Sync the yard first.")
    else:
        vi=st.selectbox(
            "Donor car",
            list(st.session_state.inventory.index),
        ...
        car=st.session_state.inventory.loc[vi]
        stock=str(car.get("stock_number") or "")
        vp=vin_profile(car)
        eng=engine_display(car)
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Trim",str((vp or {}).get("trim") or (vp or {}).get("series") or "Unknown"))
        c2.metric("Engine",eng or "Unknown")
        c3.metric("Drive",str((vp or {}).get("drive_type") or "Unknown"))
        c4.metric("Stock",stock or "—")

        st.markdown("#### Donor condition")
        cond_rows,_=background_rest_rows("scanner_donor_condition",{
            "select":"*","stock_number":f"eq.{stock}","limit":"1"
        },1)
        existing_cond=cond_rows[0] if cond_rows else {}
        with st.form("donor_condition_form"):
            cc1,cc2=st.columns(2)
            _imp_opts=["None","Front","Rear","Left","Right"]
            _sev_opts=["None","Light","Moderate","Heavy"]
            _imp=str(existing_cond.get("impact_location") or "None").title()
            _sev=str(existing_cond.get("severity") or "None").title()
            impact=cc1.selectbox("Main damage area",_imp_opts,index=_imp_opts.index(_imp) if _imp in _imp_opts else 0)
            sev=cc2.selectbox("Damage severity",_sev_opts,index=_sev_opts.index(_sev) if _sev in _sev_opts else 0)
            dc1,dc2,dc3=st.columns(3)
            flood=dc1.checkbox("Flood",value=bool(existing_cond.get("flood",False)))
            fire=dc2.checkbox("Fire",value=bool(existing_cond.get("fire",False)))
            stripped=dc3.checkbox("Interior stripped",value=bool(existing_cond.get("interior_stripped",False)))
            cnotes=st.text_input("Condition notes",value=str(existing_cond.get("notes") or ""))
            save_cond=st.form_submit_button("SAVE DONOR CONDITION")
        if save_cond:
            crow={
                "stock_number":stock,"vin":str(car.get("vin") or ""),
                "impact_location":impact,"severity":sev,"flood":flood,"fire":fire,
                "interior_stripped":stripped,"notes":cnotes.strip(),
                "updated_at":datetime.utcnow().isoformat()+"Z"
            }
            ok,msg=background_rest_upsert("scanner_donor_condition",crow,"stock_number")
            if ok: st.success("Donor condition saved. The next background run will use it.")
            else: st.error(msg)

        fid=discovery_family_id_local(car)
        family_parts=[
            r for r in discovered_parts
            if str(r.get("family_id") or "")==fid and not discovery_title_hard_blocked(r.get("part_name",""))
        ]
        donor_recs=[
            r for r in recs
            if str(r.get("stock_number") or "")==stock and not discovery_title_hard_blocked(r.get("part_name",""))
        ]
        if family_parts:
            fdf=pd.DataFrame(family_parts).sort_values(["dwmt_score","expected_net"],ascending=False)
            st.markdown("#### Everything V10.5 discovered for this donor configuration")
            st.caption("Hidden/uncertain option parts are shown here for inspection, but they do not enter Best Parts until VIN evidence or your physical verification confirms them.")
            fcols=[c for c in ["part_name","fitment_status","fitment_reason","detected_oem_part_number","oem_match_status","market_basis","sold_count_90","active_count","sell_through_pct","saturation_status","market_value","lkq_cost","expected_net","profit_per_hour","pull_minutes","dwmt_score","confidence","price_confidence_grade","walkaway_reason","price_source_url","cost_basis"] if c in fdf.columns]
            st.dataframe(
                fdf[fcols],use_container_width=True,hide_index=True,
                column_config={"price_source_url":st.column_config.LinkColumn("Price source",display_text="Open comp")}
            )

            part_choices=fdf["part_name"].astype(str).tolist()
            with st.form("donor_verify_form"):
                pname=st.selectbox("Part to verify",part_choices)
                status=st.selectbox("What did you see on the car?",["present","absent","damaged","unknown"])
                oem=st.text_input("OEM part number (optional)")
                notes=st.text_input("Notes (optional)")
                save=st.form_submit_button("SAVE DONOR VERIFICATION",type="primary")
            if save:
                prow=fdf[fdf["part_name"].astype(str).eq(str(pname))].iloc[0]
                row={
                    "id":hashlib.sha1(f"{stock}|{str(prow.get('part_key') or pname)}".lower().encode()).hexdigest(),
                    "stock_number":stock,"vin":str(car.get("vin") or ""),"part_key":str(prow.get("part_key") or pname),
                    "part_name":str(pname),"status":status,"oem_part_number":oem.strip(),"notes":notes.strip(),
                    "updated_at":datetime.utcnow().isoformat()+"Z"
                }
                ok,msg=background_rest_upsert("scanner_donor_verifications",row,"id")
                if ok:
                    st.success("Saved. The next background run will use this physical verification when rebuilding recommendations.")
                else:
                    st.error(msg)
        else:
            st.info("This exact donor configuration has not been discovery-scanned yet. New/unscanned families are prioritized automatically.")

        if donor_recs:
            ddf=pd.DataFrame(donor_recs).sort_values(["dwmt_score","expected_net"],ascending=False)
            st.markdown("#### Currently approved discovery pulls")
            cols=[c for c in ["part_name","fitment_status","fitment_reason","market_basis","sold_count_90","active_count","saturation_status","market_value","lkq_cost","expected_net","profit_per_hour","pull_minutes","dwmt_score","confidence","price_confidence_grade","oem_match_status","donor_condition_reason","learning_adjustment","price_source_url","oem_part_number"] if c in ddf.columns]
            st.dataframe(
                ddf[cols],use_container_width=True,hide_index=True,
                column_config={"price_source_url":st.column_config.LinkColumn("Price source",display_text="Open comp")}
            )

    with st.expander("How V10.4 decides what is high-selling"):
        st.write(
            "When sold-comps are available in your saved app memory, V10.4 calculates a 90-day sold count, sell-through estimate and days-to-sell. "
            "When sold data is not available, eBay Best Match rank and active-listing frequency are only used as a lower-confidence demand proxy. "
            "The app does not pretend active asking prices are completed sales."
        )

# ---------- EBAY FULL-CATALOG SWEEP ----------
if page=="🧮 EBAY SWEEP":
    st.subheader("Full LKQ price list × eBay")
    st.caption(
        "This page checks every usable Wichita LKQ price-list category against active USED eBay listings "
        "for every unique year/make/model currently in the yard. Identical donor cars share one search."
    )

    cid,secret=ebay_credentials()
    ebay_ok=bool(cid and secret)
    prices=catalog_price_rows()
    fam=catalog_donor_families()

    if not ebay_ok:
        st.warning("Connect eBay first. Add EBAY_CLIENT_ID and EBAY_CLIENT_SECRET in Streamlit Secrets.")
    if len(prices)==0:
        st.warning("Sync SETUP → LKQ prices first.")
    if len(fam)==0:
        st.warning("Sync the Wichita inventory first.")

    if len(prices) and len(fam):
        reset_catalog_sweep_if_needed()
        total=catalog_total_queries()
        cursor=min(int(st.session_state.catalog_sweep_cursor),total)
        pct=(cursor/total*100) if total else 0

        a,b,c,d=st.columns(4)
        a.metric("LKQ part rows",len(prices))
        b.metric("Unique donor families",len(fam))
        c.metric("Search combinations",f"{total:,}")
        d.metric("Completed",f"{pct:.1f}%")
        st.progress(min(1.0,pct/100 if total else 0))

        source=st.session_state.get("lkq_price_source","")
        if "fallback" in source.lower():
            st.warning(
                "Your LKQ price source is currently the fallback snapshot, so this sweep covers only those loaded rows. "
                "When the live Wichita price table loads, V9 automatically expands the sweep to the full catalog."
            )
        else:
            st.success(f"LKQ price source: {source or 'loaded price table'}")

        with st.form("catalog_sweep_form"):
            c1,c2=st.columns(2)
            batch=c1.slider("eBay searches this batch",10,250,50,10)
            min_listings=c2.slider("Minimum eBay listings for ranking",1,10,2,1)
            run_batch=st.form_submit_button("▶️ RUN NEXT EBAY SWEEP BATCH",type="primary")

        if run_batch:
            if not ebay_ok:
                st.error("eBay is not connected yet.")
            else:
                prog=st.progress(0.0)
                stat=st.empty()
                try:
                    n,stop,total,batch_results=run_catalog_sweep_batch(batch,prog,stat)
                    prog.empty()
                    stat.empty()
                    st.success(f"Checked {n} combinations. Progress: {stop:,} / {total:,}.")
                except Exception as e:
                    prog.empty()
                    stat.empty()
                    st.error(str(e))
        ...
        c1,c2=st.columns(2)
        if c1.button("🔄 RESET FULL SWEEP"):
            reset_catalog_sweep_if_needed(force=True)
            st.success("Full catalog sweep reset.")
        if c2.button("☁️ SAVE SWEEP PROGRESS"):
            try:
                save_cloud_memory()
                st.success("Sweep progress saved to Supabase.")
            except Exception as e:
                st.error(str(e))

        results=catalog_best_results(include_excluded=False,min_results=min_listings)
        if len(results):
            st.markdown("### Best LKQ → eBay spreads found so far")
            show=results.head(100).copy()
            cols=[
                "YEAR","MAKE","MODEL","LKQ_PART","DONORS","LKQ_PRICE",
                "EBAY_MEDIAN","EBAY_N","SHIP_EST","EST_NET","ROI","SHIP_CLASS"
            ]
            st.dataframe(
                show[cols],
                ...
                hide_index=True,
                column_config={
                    "LKQ_PRICE":st.column_config.NumberColumn("LKQ price",format="$%.2f"),
                    "EBAY_MEDIAN":st.column_config.NumberColumn("eBay median",format="$%.2f"),
                    "SHIP_EST":st.column_config.NumberColumn("Ship est.",format="$%.2f"),
                    "EST_NET":st.column_config.NumberColumn("Est. net",format="$%.2f"),
                    "ROI":st.column_config.NumberColumn("ROI",format="%.1fx"),
                }
            )

            st.markdown("### Matching Wichita donor cars")
            pick=st.selectbox(
                "Profit opportunity",
                list(show.index),
                format_func=lambda i:(
                    f"{show.loc[i,'YEAR']} {show.loc[i,'MAKE']} {show.loc[i,'MODEL']} — "
                    f"{show.loc[i,'LKQ_PART']} — ~{money(show.loc[i,'EST_NET'])} net"
                ),
                key="catalog_result_pick"
            )
            rr=show.loc[pick]
            donors=map_catalog_result_to_donors(rr)
            if len(donors):
                dcols=[c for c in [
                    "year","make","model","yard_location","available_date","days_in_yard",
                    "stock_number","vin"
                ] if c in donors.columns]
                st.dataframe(donors[dcols],use_container_width=True,hide_index=True)

            st.download_button(
                "⬇️ DOWNLOAD BEST SWEEP RESULTS",
                results.to_csv(index=False).encode(),
                "wichita_lkq_ebay_sweep_results.csv",
                "text/csv"
            )

            if st.toggle("Show categories scanned but excluded from pull recommendations",value=False):
                allres=catalog_best_results(include_excluded=True,min_results=min_listings)
                excluded=allres[allres.get("EXCLUDED_REC",False).astype(bool)] if len(allres) else pd.DataFrame()
                if len(excluded):
                    st.dataframe(excluded.head(100),use_container_width=True,hide_index=True)
        else:
            st.info("Run a sweep batch. Profitable eBay/LKQ comparisons will appear here.")

        st.caption(
            "Estimated net subtracts the loaded LKQ base price, your eBay fee assumption, and a category-based shipping allowance. "
            "The sweep may research large parts, but engines, transmissions, and bulky/high-shipping categories are excluded from recommended pulls by default. "
            "It uses active USED eBay asking prices, not completed-sale prices."
        )

# ---------- AUTOPILOT ----------
if page=="🤖 AUTOPILOT":
    st.subheader("V10 Autopilot")
    st.caption("Background scanning lives outside Streamlit so it keeps working when this app is closed. V10.4 re-syncs the whole Wichita yard twice daily, prices new arrivals first, progressively saves the curated market sweep, and runs car-first eBay discovery for profitable parts that are not in the original catalog.")

    calls=st.session_state.get("api_calls_total",{"ebay":0,"shopping":0})
    bg_runs,bg_run_status=background_rest_rows("scanner_runs",{
        "select":"*","order":"started_at.desc","limit":"1"
    },1)
    bg_recs,bg_rec_status=background_rest_rows("scanner_recommendations",{
        "select":"*","order":"dwmt_score.desc,expected_net.desc","limit":"100"
    },100)
    latest=bg_runs[0] if bg_runs else {}

    a,b,c,d=st.columns(4)
    a.metric("Background backend","ONLINE" if bg_runs else "SETUP NEEDED")
    b.metric("Cloud yard coverage",f"{_safe_float(latest.get('market_coverage_pct'),0):.1f}%" if bg_runs else "—")
    c.metric("Cloud market scanned",f"{int(_safe_float(latest.get('market_scanned'),0)):,}/{int(_safe_float(latest.get('market_total'),0)):,}" if bg_runs else "—")
    d.metric("Priced combinations",f"{int(_safe_float(latest.get('market_priced'),0)):,}" if bg_runs else "—")

    if not bg_runs:
        st.warning("The V10 frontend is ready, but the background database/workflow has not reported a run yet. The ZIP includes `supabase_v10.sql`, `background_scan.py`, and the GitHub Actions workflow.")
    else:
        st.success(
            f"Last run {latest.get('started_at','—')} · {latest.get('cars_found',0)} cars · "
            f"{latest.get('new_cars',0)} new · VIN {int(_safe_float(latest.get('vin_decoded'),0))}/{int(_safe_float(latest.get('vin_total'),0))} decoded · "
            f"{latest.get('ebay_calls',0)} eBay calls · cloud market {_safe_float(latest.get('market_coverage_pct'),0):.1f}% complete · "
            f"{int(_safe_float(latest.get('discovery_parts_found'),0))} new discovery clusters this run."
        )
        if latest.get("error"):
            st.error(str(latest.get("error")))

    st.markdown("### Alert rules")
    with st.form("v10_alert_rules"):
        c1,c2,c3=st.columns(3)
        min_part=c1.number_input("Minimum expected net / part",0.0,2000.0,float(st.session_state.alert_min_part_profit),25.0)
        min_donor=c2.number_input("Minimum donor opportunity",0.0,10000.0,float(st.session_state.alert_min_donor_profit),50.0)
        min_dwmt=c3.slider("Minimum DWMT score",0,100,int(st.session_state.alert_min_dwmt),5)
        if st.form_submit_button("SAVE APP ALERT RULES",type="primary"):
            st.session_state.alert_min_part_profit=min_part
            st.session_state.alert_min_donor_profit=min_donor
            st.session_state.alert_min_dwmt=min_dwmt
            mark_memory_dirty("alert rules")
            st.success("Saved in app memory. Match these values in the GitHub workflow secrets if you want the background worker to use the same thresholds.")

    st.markdown("### Background recommendations")
    if bg_recs:
        bgdf=pd.DataFrame(bg_recs)
        cols=[c for c in ["car","trim","series","engine_model","displacement_l","fuel_type","turbo","stock_number","yard_location","part","engine_fit","engine_fit_reason","vin_fit","vin_fit_reason","lkq_cost","recommended_sell_price","market_median","expected_net","pull_minutes","dwmt_score","confidence","price_confidence_grade","donor_condition_reason","learning_adjustment","price_source_url","created_at"] if c in bgdf.columns]
        st.dataframe(bgdf[cols],use_container_width=True,hide_index=True)
    else:
        st.info("No background recommendations yet.")

    st.markdown("### V10.5 car-first discoveries")
    disc_recs,disc_status=background_rest_rows("scanner_discovered_recommendations",{
        "select":"*","order":"dwmt_score.desc,expected_net.desc","limit":"100"
    },100)
    if disc_recs:
        ddf=pd.DataFrame(disc_recs)
        dcols=[c for c in ["car","trim","engine_label","stock_number","yard_location","part_name","market_basis","sold_count_90","active_count","saturation_status","lkq_cost","market_value","expected_net","profit_per_hour","pull_minutes","dwmt_score","confidence","price_confidence_grade","oem_match_status","donor_condition_reason","learning_adjustment","price_source_url","fitment_status"] if c in ddf.columns]
        st.dataframe(ddf[dcols],use_container_width=True,hide_index=True)
    else:
        st.info("No car-first discovery recommendations yet. They will populate as the background worker scans exact donor families.")

    st.markdown("### Precomputed quick-pull board")
    c1,c2=st.columns([1,1])
    if c1.button("⚡ REBUILD PRECOMPUTED BOARD",type="primary"):
        try:
            snap=refresh_precomputed_recommendations()
            st.success(f"Built {len(snap)} ranked quick-pull opportunities.")
        except Exception as e:
            st.error(str(e))
    c2.metric("Last precompute",str(st.session_state.get("precomputed_at","Never")))

    snap=pd.DataFrame(st.session_state.get("precomputed_recommendations",[]))
    if len(snap):
        show=snap[
            (pd.to_numeric(snap.get("DWMT_SCORE",0),errors="coerce")>=float(st.session_state.alert_min_dwmt)) &
            (pd.to_numeric(snap.get("EXPECTED_PROFIT",0),errors="coerce")>=float(st.session_state.alert_min_part_profit))
        ].copy()
        cols=[c for c in ["CAR","YARD_LOCATION","DAYS_YARD","ENGINE","FUEL","PART","ENGINE_FIT","DWMT","DWMT_SCORE","BUY","TARGET","EXPECTED_PROFIT","CONF","COMP_COUNT","PRICE_SPREAD_PCT","PRESENT_PCT","STOCK"] if c in show.columns]
        st.dataframe(show[cols].head(100),use_container_width=True,hide_index=True)
    else:
        st.info("Run MARKET first, then rebuild the board. V10 stores this compact board so you can open the app and see the best priced pulls immediately.")

    with st.expander("Price history / market stability"):
        hist=pd.DataFrame(st.session_state.get("price_history",[]))
        if len(hist):
            st.caption(f"{len(hist)} saved market observations (capped at 5,000).")
            keys=hist["key"].dropna().astype(str).unique().tolist()
            chosen=st.selectbox("Vehicle / part",keys,format_func=lambda x:x.replace("|"," · ")) if keys else None
            if chosen:
                h=hist[hist["key"].astype(str).eq(str(chosen))].copy()
                h["timestamp"]=pd.to_datetime(h["timestamp"],errors="coerce")
                h=h.sort_values("timestamp")
                if len(h):
                    chart=h.pivot_table(index="timestamp",columns="provider",values="median",aggfunc="last")
                    st.line_chart(chart)
                    st.dataframe(h.tail(30),use_container_width=True,hide_index=True)
        else:
            st.info("Price history starts recording automatically as V10 performs new market lookups.")

    st.markdown("### Recommendation audit trail")
    audit_rows,audit_status=background_rest_rows("scanner_recommendation_audit",{
        "select":"id,source_kind,stock_number,part_name,pricing_version,evidence,created_at",
    ...
    if audit_rows:
        ...
        for x in audit_rows:
            ...
            ar.append({
                "created_at":x.get("created_at"),"source":x.get("source_kind"),
                "stock":x.get("stock_number"),"part":x.get("part_name"),
                "engine":ev.get("engine_label"),"OEM":ev.get("oem_part_number"),
                "recommended":ev.get("recommended_sell_price"),"LKQ cost":ev.get("lkq_cost"),
                "expected net":ev.get("expected_net"),"grade":ev.get("confidence_grade"),
                "method":ev.get("pricing_method"),"price source":ev.get("source_url"),
                "why fit":ev.get("fitment_reason"),"condition":ev.get("donor_condition_reason"),
                "learning":ev.get("learning_adjustment")
            })
        adf=pd.DataFrame(ar)
        st.dataframe(adf,use_container_width=True,hide_index=True,
            column_config={"price source":st.column_config.LinkColumn("Price source",display_text="Open evidence")})
    else:
        st.caption("Audit snapshots will appear after the V10.5 background worker completes a run.")

    with st.expander("System health"):
        rows=[
            {"System":"Wichita inventory","Status":"OK" if len(st.session_state.inventory) else "NEEDS SYNC","Detail":f"{len(st.session_state.inventory)} cars"},
            {"System":"eBay","Status":"CONFIGURED" if ebay_credentials()[0] and ebay_credentials()[1] else "MISSING","Detail":"Production OAuth credentials"},
            {"System":"Google Shopping","Status":"CONFIGURED" if serpapi_key() else "OPTIONAL","Detail":"SerpAPI"},
            {"System":"Supabase memory","Status":"CONFIGURED" if cloud_memory_enabled() else "MISSING","Detail":supabase_config_source()},
            {"System":"Background tables","Status":"OK" if bg_runs else "NOT READY","Detail":bg_run_status},
            {"System":"Cloud whole-yard market","Status":"RUNNING" if bg_runs and _safe_float(latest.get("market_coverage_pct"),0)<100 else ("COMPLETE" if bg_runs else "NOT READY"),"Detail":f"{_safe_float(latest.get('market_coverage_pct'),0):.1f}% attempted" if bg_runs else "Run V10.5 background workflow"},
        ]
...
if page=="📊 DASHBOARD":
    ...
    x=purchase_profit_table()
    learning_rows,learning_status=background_rest_rows("scanner_sales_learning",{
        "select":"part_key,sales,avg_actual_profit,avg_days_to_sell,actual_to_predicted_ratio,learning_factor,updated_at",
        "order":"sales.desc","limit":"100"
    },100)
    if learning_rows:
        with st.expander("What V10.5 learned from my actual sales"):
            st.dataframe(pd.DataFrame(learning_rows),use_container_width=True,hide_index=True)
    if len(x):
...
if page=="⚙️ SETUP":
    ...
    SETUP_PAGES=["Market APIs","Speed & Cache","Memory","LKQ prices","Fitment","Impact","Shipping","Watchlist","Autopilot","Backup"]
    setup_page=st.radio(
        "Setup section",
        SETUP_PAGES,
        horizontal=True,
    ...
    if setup_page=="Market APIs":
        ...
        if secret_ebay:
            st.info("V9 full LKQ × eBay sweep is ready under 🧮 EBAY SWEEP.")
        if active:
    ...
    if setup_page=="Speed & Cache":
        ...
        st.caption("V10 fast UI: Market controls render before scoring, donor inspection scores one car only, BEST CARS skips unpriced combinations, startup never waits on Supabase, and local memory uses fast binary serialization.")
    ...
    if setup_page=="Memory":
        ...
        st.session_state.memory_autosave=st.toggle(
            "Fast local auto-save",value=st.session_state.memory_autosave,
            help="Normal page changes never wait on Supabase. Use SAVE TO CLOUD NOW for restart-proof persistence."
        )
    ...
    if setup_page=="Fitment":
        st.markdown("#### VIN / trim fitment")
        st.caption("V10.4 uses NHTSA vPIC trim/series and safety-equipment decoding from the cloud background scanner. VIN decoding cannot prove every factory package, so premium-option parts are deliberately conservative.")
        with st.form("vin_fitment_settings_v102"):
            strict=st.toggle(
                "Strict VIN Fitment (recommended)",
                value=bool(st.session_state.strict_vin_fitment),
                help="Hides navigation screens, amplifiers, power-fold mirrors, radar/ADAS parts and obvious LED mismatches unless VIN/trim evidence supports them."
            )
            save_fit=st.form_submit_button("SAVE VIN FITMENT RULE",type="primary")
        if save_fit:
            st.session_state.strict_vin_fitment=bool(strict)
            st.session_state["_fast_candidate_cache"]={}
            invalidate_rankings()
            mark_memory_dirty("VIN fitment setting")
            st.success("VIN fitment rule saved.")

        c1,c2,c3=st.columns(3)
        cache=refresh_cloud_vin_cache()
        c1.metric("Decoded VINs in cloud",len(cache))
        c2.metric("VIN cache status",st.session_state.get("_vin_decode_status","—"))
        c3.metric("Strict filtering","ON" if st.session_state.strict_vin_fitment else "OFF")
        if st.button("↻ REFRESH VIN DATA FROM CLOUD"):
            refresh_cloud_vin_cache(force=True)
            st.session_state["_fast_candidate_cache"]={}
            invalidate_rankings()
            st.success("VIN decode cache refreshed from Supabase.")

        if len(st.session_state.inventory):
            vi=st.selectbox("Inspect decoded donor",list(st.session_state.inventory.index),
                format_func=lambda i:f"{st.session_state.inventory.loc[i,'year']} {st.session_state.inventory.loc[i,'make']} {st.session_state.inventory.loc[i,'model']} — {st.session_state.inventory.loc[i].get('vin','')}",
                key="vin_fitment_vehicle")
            vd=st.session_state.inventory.loc[vi].to_dict()
            prof=vin_profile(vd)
            if prof:
                st.write({
                    "VIN":vd.get("vin",""),"Trim":prof.get("trim",""),"Series":prof.get("series",""),
                    "Body":prof.get("body_class",""),"Drive":prof.get("drive_type",""),
                    "Engine":prof.get("engine_model",""),"Displacement L":prof.get("displacement_l",""),
                    "Cylinders":prof.get("engine_cylinders",""),"Fuel":prof.get("fuel_type",""),
                    "Turbo":prof.get("turbo",""),
                    "Transmission":prof.get("transmission_style","")
                })
                fit_rows=[]
                for part in sorted(OPTION_SENSITIVE_PARTS):
                    f=vin_part_fitment(vd,part)
                    fit_rows.append({"Part":part,"Recommend":bool(f["allowed"]),"VIN fit":f["status"],"Why":f["reason"]})
                st.dataframe(pd.DataFrame(fit_rows),use_container_width=True,hide_index=True)
            else:
                st.warning("This VIN has not been decoded into the cloud cache yet. Run the V10.4 background workflow once after installing the SQL upgrade.")
    ...
    if setup_page=="Shipping":
        ...
        with st.form("quick_pull_strategy_v92"):
            qp=st.toggle(
                "Quick-Pull Only (recommended)",
                value=bool(st.session_state.quick_pull_only),
                help="Hard-blocks complete engines/transmissions and hides bulky, slow, expensive-to-ship parts from recommendations."
            )
            q1,q2=st.columns(2)
            max_min=q1.slider("Maximum pull time",15,180,int(st.session_state.max_pull_minutes),5)
            max_weight=q2.slider("Maximum part weight (lb)",20,200,int(st.session_state.max_part_weight),5)
            q3,q4=st.columns(2)
            max_dim=q3.slider("Maximum longest dimension (in)",18,84,int(st.session_state.max_part_dimension),3)
            max_ship=q4.slider("Maximum estimated shipping",20,200,int(st.session_state.max_ship_estimate),5)
            save_qp=st.form_submit_button("SAVE QUICK-PULL RULES",type="primary")
        if save_qp:
            st.session_state.quick_pull_only=qp
            st.session_state.max_pull_minutes=max_min
            st.session_state.max_part_weight=float(max_weight)
            st.session_state.max_part_dimension=float(max_dim)
            st.session_state.max_ship_estimate=float(max_ship)
            st.session_state["_fast_candidate_cache"]={}
            invalidate_rankings()
            mark_memory_dirty("quick-pull rules")
            st.success("Quick-Pull recommendation rules saved.")

        st.caption("Complete engines and complete transmissions are always excluded from recommendations, even if Quick-Pull Only is turned off.")
    ...
    if setup_page=="Autopilot":
        st.markdown("#### V10 background scanner")
        st.caption("V10.4 runs from GitHub Actions twice daily. Every run re-syncs the entire Wichita yard, detects new stock, prices new arrivals first, continues the persistent whole-yard quick-pull market sweep, and stores the data in Supabase.")
        st.code(
            "1. Run supabase_v10_1_upgrade.sql in Supabase SQL Editor\n"
            "2. Add GitHub repository secrets\n"
            "3. Commit background_scan.py + .github/workflows/background-scanner.yml\n"
            "4. Run the workflow once manually\n"
            "5. Open AUTOPILOT to verify the last run",
            language="text"
        )
        st.caption("Never put SUPABASE_SERVICE_ROLE_KEY or a Telegram bot token in Streamlit code. Keep backend-only secrets in GitHub Actions secrets.")
...
st.markdown("---")
st.caption("V10.5 is a decision aid, not a guarantee. NHTSA VIN decoding can identify trim/series and many VIN-coded safety/drivetrain variables, but it does not prove every factory option; physically verify navigation, premium audio, lamp technology, mirrors and exact OEM numbers before buying. Car-first Discovery uses imported sold comps when available; otherwise eBay Best Match/active inventory is only a lower-confidence demand proxy, not proof of completed sales. DWMT is a prioritization score, not a guarantee of condition or sale.")
