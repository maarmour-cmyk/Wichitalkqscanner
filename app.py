...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V10",
    page_icon="🔧",
    layout="wide",
...
def quick_pull_check(part_name,weight=None,length=None,width=None,height=None,pull_minutes=None,ship_est=None):
    ...
    try:
        if pull_minutes is not None and float(pull_minutes)>float(st.session_state.get("max_pull_minutes",60)):
            return False,f"Pull time > {int(st.session_state.get('max_pull_minutes',60))} min"
    except Exception:
        pass
...
def all_opportunities(inv,allow_unpriced=False):
    ...
    expected_cols=[
        ...
        "RECOMMENDABLE","QUICK_PULL_REASON",
...
def fast_research_candidates(inv):
    ...
    expected=[
        ...
        "AVAILABLE","DAYS_YARD","PART","BUY","MIN","SCORE","EXPECTED_PROFIT","PRESENT_PCT","RECOMMENDABLE","QUICK_PULL_REASON"
    ]
    if inv is None or len(inv)==0:
...
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V10</span></div>
            ...
            for _,car in summary.iterrows():
                ...
                for _,r in parts.iterrows():
                    ph+=(
                        ...
                        f"<span class='small'>{r['DWMT']} {r['DWMT_SCORE']:.0f}/100 · {r['PRESENT_PCT']:.0f}% likely present · "
    ...
    if not len(st.session_state.inventory):
        ...
        if not len(good):
            ...
            if st.toggle("Show full opportunity table",value=False,key="show_full_best_table"):
                cols=["NEW","CAR","YARD_LOCATION","AVAILABLE","DAYS_YARD","PART","DWMT","DWMT_SCORE","REC","BUY","MARKET","TARGET","CHANNEL","PROFIT","EXPECTED_PROFIT","PRESENT_PCT","$/HR","DAYS","CONF","SCORE","VALUE_SOURCE","STOCK"]
...
if page=="🌐 MARKET":
    ...
    if not len(st.session_state.inventory):
        ...
        st.caption(
            "The scan control loads immediately. V10 prioritizes fresh arrivals and quick-pull parts, then round-robins across donors so one car cannot consume the batch."
        )
        ...
        if len(inspect):
            cols=[c for c in ["PART","DWMT","DWMT_SCORE","MARKET_DATA","MARKET","TARGET","PROFIT","EXPECTED_PROFIT","CONF","COMP_COUNT","PRICE_SPREAD_PCT","VALUE_STABILITY","PRESENT_PCT","SCORE","VALUE_SOURCE"] if c in inspect.columns]
...
if page=="🤖 AUTOPILOT":
    ...
    st.caption("Background scanning lives outside Streamlit so it keeps working when this app is closed. The included GitHub Action runs hourly and writes results into Supabase.")
    ...
    b.metric("Last background run",str(latest.get("status","—")).upper())
    c.metric("Local eBay calls",int(calls.get("ebay",0)))
    d.metric("Local Shopping calls",int(calls.get("shopping",0)))
    ...
    if not bg_runs:
        ...
        st.success(
            ...
            f"{latest.get('new_cars',0)} new · {latest.get('ebay_calls',0)} eBay calls · {latest.get('alerts_sent',0)} alerts."
        )
        if latest.get("error"):
    ...
    if bg_recs:
        ...
        cols=[c for c in ["car","stock_number","yard_location","part","lkq_cost","market_median","expected_net","pull_minutes","dwmt_score","confidence","created_at"] if c in bgdf.columns]
    ...
    if len(snap):
        ...
        cols=[c for c in ["CAR","YARD_LOCATION","DAYS_YARD","PART","DWMT","DWMT_SCORE","BUY","TARGET","EXPECTED_PROFIT","CONF","COMP_COUNT","PRICE_SPREAD_PCT","PRESENT_PCT","STOCK"] if c in show.columns]
...
if page=="⚙️ SETUP":
    ...
    SETUP_PAGES=["Market APIs","Speed & Cache","Memory","LKQ prices","Impact","Shipping","Watchlist","Autopilot","Backup"]
    setup_page=st.radio(
        "Setup section",
        SETUP_PAGES,
        horizontal=True,
    ...
    if setup_page=="Autopilot":
        ...
        st.caption("The background worker is included in the V10 ZIP. It runs from GitHub Actions, detects new Wichita arrivals, prices only quick-pull parts, stores recommendations in Supabase, and can send Telegram alerts.")
        st.code(
            "1. Run supabase_v10.sql in Supabase SQL Editor\n"
...
st.markdown("---")
st.caption("V10 is a decision aid, not a guarantee. Verify physical condition, exact OEM number/interchange, programming requirements, shipping restrictions, yard price/core/fees and current marketplace rules before buying. Internet MARKET values may be active asking prices unless sold comps are imported. DWMT is a prioritization score, not a guarantee of condition or sale.")
