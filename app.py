...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V8",
    page_icon="🔧",
    layout="wide",
...
def get_opportunities_cached(inv,allow_unpriced=False):
    """
    Full-yard scoring is one of the most expensive operations in the app.
    Cache it in session state and invalidate only after inventory/market/condition
    changes or when scoring assumptions change.
    """
    ...
    key=(
        bool(allow_unpriced),
        int(st.session_state.get("ranking_revision",0)),
        str(st.session_state.get("last_sync","")),
        len(inv),
        _ranking_settings_signature(),
    )
    if key not in cache:
        cache.clear()  # keep memory small: at most the current priced/unpriced pair
...
def autosave_memory_if_changed():
    """
    Fast autosave:
    - don't serialize state on every Streamlit rerun
    - local save only when state changed
    - Supabase save at most once per configured interval
    """
    if not st.session_state.get("memory_autosave",True):
        return

    now=time.time()
    check_interval=float(st.session_state.get("memory_check_seconds",3))
    last_check=float(st.session_state.get("_last_memory_check_ts",0))
    if now-last_check < check_interval:
        return
    st.session_state["_last_memory_check_ts"]=now
    ...
    # Local write is quick and gives immediate protection during this Streamlit instance.
    try:
        save_local_memory()
    except Exception:
        pass
    ...
    cloud_interval=float(st.session_state.get("cloud_autosave_seconds",60))
    ...
    if now-last_cloud < cloud_interval:
        ...
        st.session_state.memory_status=f"Local memory saved. Cloud save queued (≤{remaining}s)."
        return
...
defaults={
    ...
    "memory_check_seconds":3,"cloud_autosave_seconds":60,
...
_cloud="Cloud memory" if cloud_memory_enabled() else "Local memory"
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V8</span></div>
      ...
      <div class="chips">
        ...
        <span class="chip">⚡ {len(st.session_state.market_cache)} cached price searches</span>
    ...
    if not len(st.session_state.inventory):
        ...
        if plan is None or not len(plan):
            ...
            for _,car in summary.iterrows():
                ...
                st.markdown(
                    f"<div class='plan-card'><div class='kpi'>Row {car.ROW} / Space {car.SPACE} — {car.CAR}</div>"
                    f"<div class='small'>Stock {car.STOCK} · {int(car.PARTS)} pulls · "
            ...
            if c1.button("🧭 SEND PLAN TO YARD MODE",type="primary"):
                ...
                route["done"]=False
                route["bought"]=False
                keep=["route_id","done","bought","SECTION","ROW","SPACE","CAR","STOCK","PART","OEM","BUY","TARGET","PROFIT","CHANNEL","SCORE"]
    ...
    if not len(st.session_state.inventory):
        ...
        if not len(good):
            ...
            if st.toggle("Show full opportunity table",value=False,key="show_full_best_table"):
                cols=["NEW","CAR","ROW","SPACE","PART","REC","BUY","MARKET","TARGET","CHANNEL","PROFIT","EXPECTED_PROFIT","PRESENT_PCT","AGE_DAYS","$/HR","DAYS","CONF","SCORE","VALUE_SOURCE","STOCK"]
...
if page=="🌐 MARKET":
    ...
    if len(st.session_state.inventory):
        with st.form("market_batch_form"):
            c1,c2=st.columns(2)
            ...
            price_now=st.form_submit_button("🌐 PRICE TOP OPPORTUNITIES",type="primary")
        if price_now:
            prog=st.progress(0.0)
            stat=st.empty()
            try:
                # Only build the full unpriced candidate list when the user actually asks for pricing.
                base=get_opportunities_cached(st.session_state.inventory,allow_unpriced=True).sort_values(["SCORE","PROFIT"],ascending=False)
        ...
        caropp=_all_unpriced[_all_unpriced["vehicle_id"].eq(vi)].sort_values(["SCORE","PROFIT"],ascending=False)
...
if page=="⚙️ SETUP":
    ...
    if setup_page=="Speed & Cache":
        ...
        st.caption("V8 uses lazy pages, cached rankings, parallel pricing, compact rendering and forms that don't rerun until submitted.")
        ...
        cstat1.metric("Fresh cached part searches",fresh)
    ...
    if setup_page=="Memory":
        ...
        st.session_state.cloud_autosave_seconds=st.slider(
            "Minimum time between cloud saves",30,300,int(st.session_state.cloud_autosave_seconds),30,
            help="60–120 seconds is recommended. Local memory still saves sooner."
        )
