...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V10.3",
    page_icon="🔧",
    layout="wide",
...
# it does not prove every as-built option. V10.3 therefore uses conservative
...
def vin_part_fitment(vehicle,part):
    ...
    if part=="OEM Navigation Screen":
        if "NAV" in _upper_words(ent):
            ...
            out=(False,"Trim mismatch",f"Base/fleet trim ({trim or 'decoded base trim'})",.9)
        elif premium:
            out=(True,"Trim-likely",f"Upper trim ({trim}); verify factory navigation/screen size",.65)
        else:
            out=(False if strict else True,"Package unproven",f"VIN decoded {trim or 'trim unknown'} but does not prove factory navigation",.25)
    elif part=="Amplifier":
        ...
        if any(_upper_words(x) in ent_u for x in PREMIUM_AUDIO_TERMS):
            ...
            out=(False,"Trim mismatch",f"Base/fleet trim ({trim or 'decoded base trim'}) is unlikely to have a premium amplifier",.85)
        elif premium:
            out=(True,"Trim-likely",f"Upper trim ({trim}); verify Bose/B&O/JBL/etc. label",.6)
        else:
            out=(False if strict else True,"Audio package unproven","VIN does not prove a separate premium-audio amplifier",.25)
    elif part=="Power Folding Mirror":
        if base:
            out=(False,"Trim mismatch",f"Base/fleet trim ({trim or 'decoded base trim'}) is unlikely to have power-fold mirrors",.85)
        elif premium:
            out=(True,"Trim-likely",f"Upper trim ({trim}); verify fold switch and connector",.6)
        else:
            out=(False if strict else True,"Mirror option unproven","VIN does not prove the power-fold option",.25)
        ...
        if base and strict:
            out=(False,"Trim mismatch",f"Base/fleet trim ({trim or 'decoded base trim'}); LED package not assumed",.75)
        else:
            out=(True,"Verify LED",f"VIN trim {trim or 'decoded'}; verify LED vs halogen/incandescent physically",.45)
    else:
...
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V10.3</span></div>
...
MAIN_PAGES=[
    ...
    "🌐 MARKET","🧮 EBAY SWEEP","🤖 AUTOPILOT","📦 BUSINESS","📊 DASHBOARD","⚙️ SETUP"
]
page=st.radio(
    "Page",
    MAIN_PAGES,
...
if page=="🌐 MARKET":
    ...
    if not len(st.session_state.inventory):
        ...
        st.caption(
            "This button is an optional on-demand local scan. V10.3 also runs a persistent cloud whole-yard sweep in the background, saving progress to Supabase and prioritizing new arrivals after each twice-daily yard sync."
        )
...
if page=="🤖 AUTOPILOT":
    ...
    st.caption("Background scanning lives outside Streamlit so it keeps working when this app is closed. V10.3 re-syncs the whole Wichita yard twice daily, prices new arrivals first, and progressively saves the entire quick-pull market sweep to Supabase.")
    ...
    with st.expander("System health"):
        rows=[
            ...
            {"System":"Cloud whole-yard market","Status":"RUNNING" if bg_runs and _safe_float(latest.get("market_coverage_pct"),0)<100 else ("COMPLETE" if bg_runs else "NOT READY"),"Detail":f"{_safe_float(latest.get('market_coverage_pct'),0):.1f}% attempted" if bg_runs else "Run V10.3 background workflow"},
        ]
...
if page=="⚙️ SETUP":
    ...
    if setup_page=="Fitment":
        ...
        st.caption("V10.3 uses NHTSA vPIC trim/series and safety-equipment decoding from the cloud background scanner. VIN decoding cannot prove every factory package, so premium-option parts are deliberately conservative.")
        ...
        if len(st.session_state.inventory):
            ...
            if prof:
                ...
                st.warning("This VIN has not been decoded into the cloud cache yet. Run the V10.3 background workflow once after installing the SQL upgrade.")
    ...
    if setup_page=="Autopilot":
        ...
        st.caption("V10.3 runs from GitHub Actions twice daily. Every run re-syncs the entire Wichita yard, detects new stock, prices new arrivals first, continues the persistent whole-yard quick-pull market sweep, and stores the data in Supabase.")
        st.code(
...
st.markdown("---")
st.caption("V10.3 is a decision aid, not a guarantee. NHTSA VIN decoding can identify trim/series and many VIN-coded safety/drivetrain variables, but it does not prove every factory option; physically verify navigation, premium audio, lamp technology, mirrors and exact OEM numbers before buying. Internet MARKET values may be active asking prices unless sold comps are imported. DWMT is a prioritization score, not a guarantee of condition or sale.")
