...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V9.1",
    page_icon="🔧",
    layout="wide",
...
CATALOG_EXCLUDED_RECOMMENDATION_TERMS=[
    "engine","long block","short block","wheel","rim","tire",
]
...
def catalog_recommendation_excluded(part_name):
    n=norm(part_name)
    return any(t in n for t in CATALOG_EXCLUDED_RECOMMENDATION_TERMS)

def _catalog_ebay_worker(item,token,ebay_fee):
    q=item["QUERY"]
    try:
        ...
        out={**item,
             ...
             "EXCLUDED_REC":bool(catalog_recommendation_excluded(item["LKQ_PART"])),
        ...
        return {**item,
                ...
                "SHIP_EST":catalog_shipping_guess(item["LKQ_PART"])[0],
                "SHIP_CLASS":catalog_shipping_guess(item["LKQ_PART"])[1],
...
def run_catalog_sweep_batch(batch_size=50,progress=None,status=None):
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        future_map={
            pool.submit(_catalog_ebay_worker,item,token,ebay_fee):item
            for item in tasks
        }
...
def fast_research_candidates(inv):
    ...
    expected=[
        ...
        "AVAILABLE","DAYS_YARD","PART","BUY","MIN","SCORE","EXPECTED_PROFIT","PRESENT_PCT"
    ]
    if inv is None or len(inv)==0:
...
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V9.1</span></div>
...
MAIN_PAGES=[
    ...
    "🌐 MARKET","🧮 EBAY SWEEP","📦 BUSINESS","📊 DASHBOARD","⚙️ SETUP"
]
page=st.radio(
    "Page",
    MAIN_PAGES,
...
if page=="🌐 MARKET":
    ...
    if not len(st.session_state.inventory):
        ...
        st.caption(
            "The scan control loads immediately. Whole-yard candidate ranking starts only after you press SCAN MARKET NOW."
        )
        ...
        if len(inspect):
            cols=[c for c in ["PART","MARKET_DATA","MARKET","TARGET","PROFIT","EXPECTED_PROFIT","CONF","PRESENT_PCT","SCORE","VALUE_SOURCE"] if c in inspect.columns]
...
if page=="🧮 EBAY SWEEP":
    ...
    if len(prices) and len(fam):
        ...
        st.caption(
            ...
            "It uses active USED eBay asking prices, not completed-sale prices. Exact OEM-number scans should still be preferred before purchasing."
        )
...
if page=="⚙️ SETUP":
    ...
    if setup_page=="Speed & Cache":
        ...
        st.caption("V9.1 fast UI: Market controls render before scoring, donor inspection scores one car only, BEST CARS skips unpriced combinations, startup never waits on Supabase, and local memory uses fast binary serialization.")
...
st.markdown("---")
st.caption("V9.1 is a decision aid, not a guarantee. Verify physical condition, exact OEM number/interchange, programming requirements, shipping restrictions, yard price/core/fees and current marketplace rules before buying. Internet MARKET values may be active asking prices unless sold comps are imported.")
