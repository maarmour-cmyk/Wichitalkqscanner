...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V8.6",
    page_icon="🔧",
    layout="wide",
...
# One small read-only Supabase probe per Streamlit session.
# This makes the header/status truthful without slowing every rerun.
if cloud_memory_enabled() and not st.session_state.get("_supabase_probe_done",False):
    ok,msg=test_cloud_memory_connection()
    st.session_state.supabase_connected=ok
    st.session_state.supabase_connection_status=msg
    st.session_state["_supabase_probe_done"]=True
...
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V8.6</span></div>
...
if page=="⚙️ SETUP":
    ...
    if setup_page=="Memory":
        ...
        st.session_state.cloud_autosave_seconds=st.slider(
            ...
            help="90–180 seconds is recommended."
        )
