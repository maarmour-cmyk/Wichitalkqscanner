 #!/usr/bin/env python3
"""Wichita LKQ V10.5.5 cloud market + VIN-fitment scanner.
...
DISCOVERY_RULES=[
    ...
    (r"\b(power seat motor|seat track motor|seat adjustment motor|seat adjuster motor|seat recline motor)\b","Power Seat Motor",30.0,25,12,"trim"),
    (r"\b(power seat switch|seat adjustment switch|seat adjuster switch|memory seat switch)\b","Power Seat Switch",20.0,10,9,"trim"),
    ...
    (r"\b(air suspension compressor|air ride compressor|leveling compressor)\b","Air Suspension Compressor",55.0,35,20,"trim"),
    (r"\b(suspension control module|ride control module|adaptive suspension module)\b","Suspension / Ride Control Module",40.0,20,12,"trim"),
    (r"\b(electronic parking brake module|parking brake module|epb module|parking brake actuator)\b","Electronic Parking Brake Module / Actuator",40.0,25,14,"trim"),
    ...
    (r"\b(power liftgate motor|liftgate motor|power liftgate actuator|liftgate spindle|liftgate strut motor)\b","Power Liftgate Motor / Actuator",45.0,30,18,"premium"),
    (r"\b(power sliding door motor|sliding door motor|power door motor|sliding door actuator)\b","Power Sliding Door Motor / Actuator",45.0,35,18,"trim"),
    ...
    (r"\b(convertible top motor|convertible top pump|roof hydraulic pump|soft top motor)\b","Convertible Top Motor / Hydraulic Pump",55.0,40,18,"premium"),
    ...
    (r"\b(window regulator motor|power window motor)\b","Window Regulator Motor",25.0,25,14,"body"),
    (r"\b(wiper motor|windshield wiper motor)\b","Wiper Motor",30.0,25,14,"general"),
    ...
    (r"\b(low pressure fuel pump|fuel pump assembly|fuel pump module|in.?tank fuel pump|fuel sender pump)\b","Low-Pressure Fuel Pump / Module",48.0,45,16,"engine_exact"),
    (r"\b(electric water pump|auxiliary coolant pump|aux coolant pump|coolant circulation pump)\b","Electric Water / Coolant Pump",45.0,35,15,"engine_exact"),
    (r"\b(turbo actuator|wastegate actuator|electronic wastegate)\b","Turbo / Wastegate Actuator",45.0,25,12,"turbo"),
    (r"\b(egr valve|egr actuator|egr control valve)\b","EGR Valve / Actuator",42.0,35,14,"engine_exact"),
...
def parse_card(title,blob,url):
    ...
    available=avail.group(1) if avail else ""
    age=None
    try:
        dt=parse_dt(available)
        if dt:
    ...
    return {
        ...
        "yard_location":loc,"available_date":available,"days_in_yard":age,
        "source_url":url
    }
...
def fetch_inventory(max_pages=75):
    s=requests.Session(); s.headers["User-Agent"]="Mozilla/5.0 WichitaPartsScanner/10.5.5"
...
def _market_family_key(c):
    return f"{int(c.get('year') or 0)}|{clean_upper(c.get('make'))}|{clean_upper(c.get('model'))}"

def _round_robin_candidates(rows):
    """Interleave candidate parts across vehicle families.

    This prevents a 1,500-query run from spending many searches on early
    year/make/model families while later yard cars get no market coverage.
    """
    buckets=defaultdict(list)
    family_order=[]
    seen_fams=set()
    for c in rows:
        fk=_market_family_key(c)
        if fk not in seen_fams:
            family_order.append(fk); seen_fams.add(fk)
        buckets[fk].append(c)

    # Within each family, try fastest/easiest parts first.
    for fk in buckets:
        buckets[fk].sort(key=lambda x:(
            int(x.get("pull") or 999),
            float(x.get("ship") or 999),
            str(x.get("part") or "")
        ))
...
def main():
    ...
    try:
        ...
        new_stock_set={str(c.get("stock_number") or "") for c in new_cars}
        for c in cars:
            c["is_new"]=str(c.get("stock_number") or "") in new_stock_set
        ...
        # 4) Build a coverage-first queue.
        #
        # V10.5.3 could spend several searches on one early vehicle family before
        # later families were touched. V10.5.4 first gives every year/make/model
        # family a chance at one current price, then deepens coverage round-robin.
        new_family_keys={
            family_key(c) for c in new_cars
        }

        missing=[]; stale=[]; priority_new=[]
        for c in candidates:
            row=cache.get(c["id"])
            if row is None:
                missing.append(c)
            ...
            if family_key(c) in new_family_keys and not cache_is_fresh(row,ttl):
                priority_new.append(c)

        stale=[c for _,c in sorted(stale,key=lambda x:x[0])]

        # Which families already have at least one V10.5.4/current price attempt?
        attempted_families=set()
        for c in candidates:
            row=cache.get(c["id"])
            if row and str(row.get("pricing_version") or "")==PRICING_VERSION:
                attempted_families.add(_market_family_key(c))

        unseen_family_first=[]
        deeper_missing=[]
        seen_first=set()
        for c in _round_robin_candidates(missing):
            fk=_market_family_key(c)
            if fk not in attempted_families and fk not in seen_first:
                unseen_family_first.append(c)
                seen_first.add(fk)
            else:
                deeper_missing.append(c)

        # New arrivals still lead, but only one candidate per new family is
        # promoted ahead of untouched older families. That keeps "new first"
        # without starving the rest of the yard.
        new_first=[]; new_rest=[]; seen_new=set()
        for c in _round_robin_candidates(priority_new):
            fk=_market_family_key(c)
            if fk not in seen_new:
                new_first.append(c); seen_new.add(fk)
            else:
                new_rest.append(c)

        # Deduplicate while preserving queue order.
        queue=[]; qseen=set()
        for c in (
            new_first
            + unseen_family_first
            + _round_robin_candidates(deeper_missing)
            + _round_robin_candidates(new_rest)
            + _round_robin_candidates(stale)
        ):
            if c["id"] in qseen: continue
            qseen.add(c["id"]); queue.append(c)

        budget=max(1,int(fnum("MAX_MARKET_QUERIES_PER_RUN",1500)))
        selected=queue[:budget]
        print(
            f"Coverage-first queue: {len(unseen_family_first)} untouched vehicle families, "
            f"{len(missing)} missing combos, {len(stale)} stale combos, "
        ...
        fam_budget=max(0,int(fnum("MAX_DISCOVERY_FAMILIES_PER_RUN",150)))
