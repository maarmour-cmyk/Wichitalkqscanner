#!/usr/bin/env python3
"""Wichita LKQ V10.4 cloud market + VIN-fitment scanner.
...
DISCOVERY_BLOCK_TERMS={
    "complete engine","engine assembly","long block","short block","complete transmission",
    "transmission assembly","wheel tire","wheel and tire","rim tire","set of wheels","set of rims",
...
def _blocked_discovery_title(title):
    ...
    return any(x in t for x in DISCOVERY_BLOCK_TERMS)
...
def classify_discovery_title(title,car,profile=None):
    ...
    if re.search(r"\b(module|controller|control unit|switch|sensor|actuator|solenoid|motor|pump|valve)\b",text,re.I):
        ...
        if len(phrase)>=5:
            ...
            elif re.search(r"\bactuator|motor\b",text,re.I): cost,pull,ship=35.0,20,14
...
def sold_evidence_for(car,part_name,sold_rows):
    ...
    return {"sold_median":float(statistics.median(vals)) if vals else None,"sold_count_90":n90,"sold_count_all":len(vals)}
...
def discover_family(car,profile,token,sold_rows):
    ...
    for key,items in groups.items():
        ...
        c=meta[key]; vals=sorted(i["price_total"] for i in items if i["price_total"]>0)
        if not vals: continue
        med=float(statistics.median(vals)); active=len(items)
        ...
        sold=sold_evidence_for(car,c["part_name"],sold_rows)
        market=float(sold["sold_median"] if sold["sold_median"] is not None else med)
        target=market*quick
...
def discovered_rec_for(car,profile,row,verification=None):
    ...
    if expected < fnum("DISCOVERY_MIN_EXPECTED_NET",75) or score < fnum("DISCOVERY_MIN_DWMT",45):
        return None
    ...
    return {
        ...
        "dwmt_score":score,"confidence":round(conf,1),"fitment_status":"VERIFIED" if status=="present" else "VIN/CONFIG ELIGIBLE",
    ...
    try:
        ...
        for car in cars:
            ...
            for r in by_family.get(fid,[]):
                ...
                rec=discovered_rec_for(car,profile,r,v)
