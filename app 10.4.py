...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V10.4",
    page_icon="🔧",
    layout="wide",
...
ENGINE_SENSITIVE_PARTS={"Turbocharger","Diesel High Pressure Fuel Pump","Diesel Injector Set"}
...
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V10.4</span></div>
      ...
      <div class="chips">
        ...
        <span class="chip">🎯 {_covered_now}/{len(st.session_state.inventory)} cars with market price</span>
        <span class="chip">🧠 {_cloud}</span>
        <span class="chip">🤖 V10 autopilot ready</span>
      </div>
    </div>""",
    unsafe_allow_html=True
)
    ...
    if not len(st.session_state.inventory):
        ...
        if not len(good):
            ...
            if st.toggle("Show full opportunity table",value=False,key="show_full_best_table"):
                cols=["NEW","CAR","YARD_LOCATION","AVAILABLE","DAYS_YARD","ENGINE","FUEL","PART","ENGINE_FIT","DWMT","DWMT_SCORE","REC","BUY","MARKET","TARGET","CHANNEL","PROFIT","EXPECTED_PROFIT","PRESENT_PCT","$/HR","DAYS","CONF","SCORE","VALUE_SOURCE","STOCK"]
...
if page=="🧠 DISCOVERY":
    ...
    st.caption(
        "V10.4 searches eBay from the car outward instead of only testing the built-in parts catalog. "
    ...
    if recs:
        ...
        cols=[c for c in [
            ...
            "sold_count_90","active_count","sell_through_pct","market_value","lkq_cost","expected_net",
            "profit_per_hour","pull_minutes","dwmt_score","confidence","stock_number"
        ] if c in show.columns]
        st.dataframe(show[cols].head(300),use_container_width=True,hide_index=True)
    else:
    ...
    if not len(st.session_state.inventory):
        ...
        family_parts=[r for r in discovered_parts if str(r.get("family_id") or "")==fid]
        donor_recs=[r for r in recs if str(r.get("stock_number") or "")==stock]
        if family_parts:
            fdf=pd.DataFrame(family_parts).sort_values(["dwmt_score","expected_net"],ascending=False)
            st.markdown("#### Everything V10.4 discovered for this donor configuration")
            ...
            fcols=[c for c in ["part_name","fitment_status","fitment_reason","market_basis","sold_count_90","active_count","sell_through_pct","market_value","lkq_cost","expected_net","profit_per_hour","pull_minutes","dwmt_score","confidence","cost_basis"] if c in fdf.columns]
            st.dataframe(fdf[fcols],use_container_width=True,hide_index=True)

            part_choices=fdf["part_name"].astype(str).tolist()
            with st.form("donor_verify_form"):
                pname=st.selectbox("Part to verify",part_choices)
                status=st.selectbox("What did you see on the car?",["present","absent","unknown"])
        ...
        if donor_recs:
            ...
            cols=[c for c in ["part_name","fitment_status","fitment_reason","market_basis","sold_count_90","active_count","market_value","lkq_cost","expected_net","profit_per_hour","pull_minutes","dwmt_score","confidence","oem_part_number"] if c in ddf.columns]
...
if page=="🤖 AUTOPILOT":
    ...
    if bg_recs:
        ...
        cols=[c for c in ["car","trim","series","engine_model","displacement_l","fuel_type","turbo","stock_number","yard_location","part","engine_fit","engine_fit_reason","vin_fit","vin_fit_reason","lkq_cost","market_median","expected_net","pull_minutes","dwmt_score","confidence","created_at"] if c in bgdf.columns]
        st.dataframe(bgdf[cols],use_container_width=True,hide_index=True)
    else:
        st.info("No background recommendations yet.")

    st.markdown("### V10.4 car-first discoveries")
    disc_recs,disc_status=background_rest_rows("scanner_discovered_recommendations",{
        "select":"*","order":"dwmt_score.desc,expected_net.desc","limit":"100"
    },100)
    if disc_recs:
        ddf=pd.DataFrame(disc_recs)
        dcols=[c for c in ["car","trim","engine_label","stock_number","yard_location","part_name","market_basis","sold_count_90","active_count","lkq_cost","market_value","expected_net","profit_per_hour","pull_minutes","dwmt_score","confidence","fitment_status"] if c in ddf.columns]
    ...
    with st.expander("System health"):
        rows=[
            ...
            {"System":"Cloud whole-yard market","Status":"RUNNING" if bg_runs and _safe_float(latest.get("market_coverage_pct"),0)<100 else ("COMPLETE" if bg_runs else "NOT READY"),"Detail":f"{_safe_float(latest.get('market_coverage_pct'),0):.1f}% attempted" if bg_runs else "Run V10.4 background workflow"},
        ]
