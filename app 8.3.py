...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V8.3",
    page_icon="🔧",
    layout="wide",
...
defaults={
    ...
    "memory_check_seconds":3,"cloud_autosave_seconds":90,
    "_memory_dirty":False,"_cloud_memory_pending":False,"_memory_dirty_reason":"",
...
try:
    _priced_now=get_opportunities_cached(st.session_state.inventory,allow_unpriced=False) if len(st.session_state.inventory) else pd.DataFrame()
    _covered_now=int(_priced_now["vehicle_id"].nunique()) if len(_priced_now) else 0
except Exception:
    _covered_now=0
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V8.3</span></div>
    ...
    if len(st.session_state.inventory):
        ...
        if len(remaining):
            with st.expander(f"Cars still needing a usable market price ({len(remaining)})"):
                cols=[c for c in ["year","make","model","yard_row","space","stock_number","available_date"] if c in remaining.columns]
...
if page=="⚙️ SETUP":
    ...
    if setup_page=="Speed & Cache":
        ...
        st.caption("V8.2 adds event-driven memory: ordinary page changes no longer serialize or save the whole app state.")
