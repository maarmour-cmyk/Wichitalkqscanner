...
st.set_page_config(
    page_title="Wichita Parts Profit Scanner V9",
    page_icon="🔧",
    layout="wide",
...
def _catalog_ebay_worker(item,token):
    q=item["QUERY"]
    try:
        ...
        if pd.notna(median):
            ...
            fee=float(median)*float(st.session_state.get("ebay_fee",.13))
...
def run_catalog_sweep_batch(batch_size=50,progress=None,status=None):
    ...
    tasks=[]
    for ix in range(cursor,stop):
        item=catalog_query_from_index(ix)
        if item:
            tasks.append(item)
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        future_map={pool.submit(_catalog_ebay_worker,item,token):item for item in tasks}
...
st.markdown(
    f"""<div class="hero">
      <div class="hero-title">Wichita Parts Profit Scanner <span style="color:#ffb000">V9</span></div>
...
if page=="🌐 MARKET":
    ...
    if len(st.session_state.inventory):
        ...
        b.metric("Cars with market price",cov["covered"])
        ...
        st.caption(
            "“Cars with market price” counts donors that already have at least one usable eBay/Google price. "
            "Cached searches can include zero-result searches, so cached-search count may be higher than priced-car coverage. "
            "Whole-yard mode gives every donor a chance before spending extra searches on already-priced cars."
        )
